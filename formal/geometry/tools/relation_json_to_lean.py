...
def render_evaluation() -> str:
    return "\n".join([
        "private def relationCode : SpatialRelation → String",
        '  | .portFrame _ => "port-frame"',
        '  | .properRigid _ => "proper-rigid"',
        '  | .rotatableJoint _ => "rotatable-joint"',
        "",
        "private def failedRelationCodes : List SpatialRelation → List String",
        "  | [] => []",
        "  | relation :: rest =>",
        "      let tail := failedRelationCodes rest",
        "      if spatialRelationIsSatisfied reference candidate relation then tail",
        "      else relationCode relation :: tail",
        "",
        "private def evaluationPayload : String :=",
        "  let failures := failedRelationCodes relations",
        '  let status := if failures.isEmpty then "pass" else "reject"',
        '  let encoded := String.intercalate "," (failures.map fun code => "\\\"" ++ code ++ "\\\"")',
        '  "{\\\"status\\\":\\\"" ++ status ++ "\\\",\\\"scope\\\":\\\"spatial-relation-v1\\\",\\\"issues\\\":[" ++ encoded ++ "]}"',
        "",

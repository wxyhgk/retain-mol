...
EditPlan -> schema -> builder dry-run -> 候选 Molecule
                                      |
                                      v
                   trusted orchestrator 生成 GeometryPolicy
                                      |
                                      v
                              Lean 精确验证
                                      |
...
请求中的 scale、policy 和 Lean 源码都不能由 AI 提交。距离使用平方距离，避免平方根；朝向
...
1. 从规范化 `EditPlan` 独立编译 expected effect，再与 builder receipt 比对；
2. 建立绑定最终 artifact hash 的 PASS / REJECT / INDETERMINATE envelope；

...
独立攻击测试曾构造出旧 policy 会放过的八类错误：刚体镜像、错误整体旋转、连接方向反转、
错误二面角、共面退化、非键碰撞、错误键角和过宽键长区间。当前 `AtomPortMate` 已用有向线段
点积符号和余弦平方区间封闭最小连接方向缺口；其余关系仍需逐项进入受信 policy：
...
| 二面角 | 两平面法向量点积和叉积符号 |
| 平面性 | 点到受信平面的有符号距离区间 |
| 连接方向 | 已实现最小有向线段对齐；完整端口姿态仍需局部 frame 分量区间 |

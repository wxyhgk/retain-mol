...
## 核心方法：让 AI 编写关系程序，而不是想象全局坐标

AI 的三维困难主要来自同时维护大量原子的世界坐标。RetainMol 不要求模型完成这件事，而是
把建模请求编译成一个有限的空间关系程序：

```text
StructureIntent
├─ graph rewrite       删除/增加哪些原子和键
├─ protected anchors   哪些原子绝不能移动
├─ rigid regions       哪些片段只能整体运动
├─ local port frames   连接端口的原点、轴向和径向参考
├─ mate relations      哪两个端口以何种键级、距离和朝向连接
└─ rotatable joints    哪条单键允许转动以及允许的转角区间
```

执行过程固定为：

1. 多模态 AI 从图片、文本和当前结构中识别分子图与局部关系；
2. builder dry-run 执行图改写，拒绝非法键、重复键和不存在的端点；
3. 确定性放置器只根据局部端口坐标系生成初始坐标；
4. 距离几何、MMFF/UFF 或 xTB 优化允许的自由度；
5. Lean 对最终候选验证图改写、锚点、刚性、朝向、连接距离和碰撞；
6. 失败只返回局部关系问题，AI 修改对应关系后重试，不重画整个分子。

这样，模型需要解决的是有限的离散选择和局部关系，而不是在一次推理中记住几百个三维坐标。
Lean 也不证明 AI 看懂了图片；它证明的是“候选确实满足系统拥有的关系契约”。

当前第一条可执行样板是 `AtomPortMate`：宿主碳上的指定 H 被删除，客体刚性片段加入，系统
创建指定单键，并验证客体未变形、局部手性未翻转、连接距离位于可信区间，且新键沿被替换 H
的离去方向接入。方向检查使用点积符号和余弦平方有理区间；“距离正确、刚性正确、但横向接入”
的反例会被拒绝。请求只能引用固定 registry 的 policy/evidence ID 与摘要，不能内联候选、阈值
或 Lean 源码，因此 AI 不能降低标准后自证通过。Lean 求值结果会原样携带命令 ID、投影版本、
policy/evidence ID 与摘要；消费端必须逐项匹配，不能把一个匿名 `pass` 重新关联到另一份证据。
...
- 精确整数 `Vec3`；
- 平移保持距离和朝向的定理；
- 完整 canonical chemical graph、固定锚点、距离区间、有向线段对齐、刚性组和朝向验证；

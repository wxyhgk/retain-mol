...
尚未实现多初态构象搜索、整体莫比乌斯带拓扑校验、路径连续防穿透、力场或量子化学计算。同步纯函数适合当前小区域演示；较大结构的交互式求解仍需要宿主放入 Worker 或后端任务，不应在主线程无限增加迭代。

没有搜索或下载具体分子的现成坐标。接下来应在这些独立约束和诊断之上扩展扭曲骨架初始化、环带区域描述及持续碰撞检查。

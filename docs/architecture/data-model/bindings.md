...
> **分子来源状态：xTB 主路径已实现。** schema 4 引入 Binding，schema 5 完成不可变 Molecule Revision；Molecule service/API、xTB 端口校验、排队摘要冻结和 runner 消费已端到端接入。历史 xTB Job 仍保留 `metadata.request` 回退。

## 为什么需要 Binding

`CalculationSpec` 描述计算方法，`JobInputBinding` 描述某一次 Job 实际读取的不可变输入。两者必须分离：相同的 xTB 参数可以用于不同分子，而一次已经排队的计算不能因为分子被继续编辑、Workflow 被改线或文件被覆盖而改变输入。
...
3. 解析时看到的内容摘要是什么；
4. 如果输入来自 Workflow，具体由哪条引用解析而来。

## schema 4 实体

数据库表目标名为 `job_input_bindings`。API 使用 camelCase，SQLite 使用对应 snake_case。
...
| `resolvedFromReferenceId` | `resolved_from_reference_id` | string? | Workflow 引用解析产生绑定时，指向原 `JobInputReference` |
...
- `resolvedFromReferenceId` 有值时，引用的目标 Job 和 `inputName` 必须与 Binding 一致。
- Binding 行创建后不可更新。正式提交采用新 Job 表达输入变化，不原地改写已经审计的绑定。
...
- 所有必需端口都存在，未知端口被拒绝；
- 单值端口只有一个 Binding；schema 4 首轮不开放多值端口；
- `sourceKind` 在端口允许列表中；
- Artifact 的 `format`、`role` 和完整性状态符合端口要求；
- MoleculeRevision 存在、摘要一致，并且能导出该引擎需要的结构格式；
- literal 能通过端口对应的 JSON schema 或 Pydantic 校验。

端口契约可以由后端 registry 提供，并在 `CalculationSpec.payload.inputPorts` 中保存创建时使用的版本化快照。runner 只消费已验证 Binding，不自行选择输入。

## 原子提交：create → bind → queue
...
sequenceDiagram
    ...
    S->>DB: INSERT explicit JobInputBinding
    S->>DB: resolve Workflow references
    S->>DB: INSERT resolved JobInputBinding
    S->>DB: validate ports, sources and digests
    S->>DB: CAS created -> queued
    S->>DB: INSERT JobStatusEvent
    S->>DB: COMMIT
    S-->>C: queued Job + frozen bindings
```
...
- 对已经创建的 Binding 执行 UPDATE；
- 为了重跑而把终态 Job 改回 `queued`。

runner 只能通过 `jobId` 读取 `CalculationSpec + JobInputBinding`，准备私有工作目录，然后以只读方式消费 Revision 或 Artifact 内容。
...
3. 复算或按完整性策略验证 `structure_json`，把 Revision ID 和当时摘要写入 Binding。
4. 与 Spec、Job 创建及 `created -> queued` 在同一事务提交。
5. runner 按 Binding 读取 Revision 并再次比对摘要，不查询 Asset head。

因此 Asset head 在 Job 排队后继续前进不会影响该 Job。Revision 也不能因为已有 Binding 而被原地修复；若历史行内容或摘要不可信，应显式迁移为新的 Revision，或拒绝排队。

完整的 create asset、save revision 和 head CAS 契约见 [MoleculeAsset / MoleculeRevision 契约](./molecule-assets.md)。

## Workflow Reference 解析

`JobInputReference` 是设计时的依赖意图，`JobInputBinding` 是排队时冻结的执行事实。引用必须在上述提交事务中解析为具体来源。
...
4. 创建 `sourceKind = artifact` 的 Binding，并写入 `artifactId`、`contentSha256` 和 `resolvedFromReferenceId`。
5. `sourceName` 只作为 legacy 回退；若同名输出不唯一则拒绝排队，不能任意选择一个。

### 上游输入引用

当 `sourceKind = input` 时，解析器读取上游 Job 已冻结的同名 Binding，把其 literal、Revision ID 或 Artifact ID 复制为新的目标 Binding，并重新记录摘要。不能复制任务目录路径，也不能让两个 Job 共享一条 Binding 行。

显式输入和 Workflow 引用若同时占用同一 `inputName`，提交失败，不设置隐式覆盖优先级。Workflow 后续编辑不影响已经生成的 Binding。

## xTB 示例

以下是当前 xTB Revision 输入在服务层冻结后的 Binding 表达；HTTP 创建任务时客户端只提交 `moleculeRevisionId`，由服务端生成 Binding ID 和摘要。
...
两种 Job 使用同一类 Spec，但其结构输入和 provenance 不同。目标 runner 对新 Job 不再从 `metadata.request.molecule` 或任务目录猜测结构；历史 Job 的只读回退继续受兼容窗口约束。

## legacy 兼容窗口

schema 4 迁移必须保留历史 Job 可读取，但所有新提交只写 Binding。

| legacy 来源 | 兼容策略 |
| --- | --- |
| `job_inputs(input_name, value_json)` | 读取适配器投影为 `sourceKind = literal`；迁移只在不猜测化学语义的前提下物化 Binding |
| `metadata.request` | xTB runner 暂时保留只读回退；新 Job 不再写入，且需要记录回退使用量 |
| `JobInputReference.sourceName` | 仅在没有 `sourceArtifactId` 时回退；必须唯一解析，否则拒绝 |
| legacy Artifact path | 先通过安全路径和内容校验解析，再绑定具体 Artifact ID；Binding 不保存 path |

兼容顺序固定为：优先读取 Spec + Binding，仅当历史 Job 缺少 Binding 时才使用 legacy adapter。禁止把 legacy 值反向覆盖已经存在的 Binding。
...
- migration 3 不修改，新增 migration 4 创建 Binding 表和约束；
- 新提交只允许通过原子的 create → bind → queue service；
- 端口缺失、来源类型错误、摘要不一致和 Workflow 冲突均保持无副作用失败；
- Job 进入 `queued` 后无法新增、删除或修改 Binding；
- xTB runner 在新 Job 上完全不依赖 `job_inputs` 或 `metadata.request`；
- 相同 Workflow 重复提交时，每个 Job 都保存独立、可追溯的 Binding；

...
- [Job 输入绑定](./bindings.md)：schema 4 的输入端口、冻结事务、Workflow 解析和 legacy 兼容目标。
...
2. **输入必须冻结**：Job 进入 `queued` 前，所有可变来源都解析成不可变的 `JobInputBinding`。
...
erDiagram
    ...
    Job ||--o{ JobInputBinding : "冻结输入"
    MoleculeRevision o|--o{ JobInputBinding : "可作为来源"
    Artifact o|--o{ JobInputBinding : "可作为来源"
    Job ||--o{ Artifact : "产生"
    Workflow }o--o{ Job : "编排"
    Workflow ||--o{ JobInputReference : "声明依赖"
    Job ||--o{ JobInputReference : "源 Job"
    Job ||--o{ JobInputReference : "目标 Job"
    JobInputReference o|--o| JobInputBinding : "解析为"
```

多态来源的约束如下：

- 一个 `JobInputBinding` 只能选择 `literal`、`molecule_revision`、`artifact` 三种来源之一。
...
- 一个 `JobInputReference` 只描述同一 Workflow 内两个 Job 之间的边；解析后得到目标 Job 的一个绑定。
...
| Job 输入 | Binding 覆盖 literal/Revision/Artifact；Revision 摘要在排队和 runner 读取时校验 | 逐步缩小历史 `metadata.request` 回退窗口 |

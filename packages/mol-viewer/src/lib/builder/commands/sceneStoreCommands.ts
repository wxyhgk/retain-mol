...
import { runSetAtomPositionsCommand } from './moleculeStoreCommands'
...
export function runSetMoleculeInSceneCommand(
  ...
  if (activeObjectId && objectsById[activeObjectId]) {
    ...
    return {
      ...
      objectsById: {
        ...objectsById,
        [activeObjectId]: { ...object, molecule, name: molecule.name ?? object.name },
      },
  ...
  const object = createSceneObject(molecule)
  return {
    ok: true,
    changed: true,

...
import { editChanged, editFailed, editUnchanged, type EditCommandResult } from './commandResult'
...
export function runCycleBondOrderCommand(
  ...
  return editChanged({
    ...
    bonds: molecule.bonds.map(candidate =>
      candidate.id === bondId ? { ...candidate, order: next, aromatic: undefined } : candidate),
  })
}

...
export class GeometryRelaxer {
  ...
  constructor(mol: Molecule, opts: RelaxOptions = {}) {
    ...
    for (let i = 0; i < n; i++) {
      this.x[i] = mol.atoms[i].x
      this.y[i] = mol.atoms[i].y
      this.z[i] = mol.atoms[i].z + (rnd() - 0.5) * 2 * jitter
    }
    ...
    if (target) {
      ...
      for (let i = 0; i < n; i++) {
        const t = target.get(this.ids[i])
        this.tx[i] = t ? t.x : this.x[i]
        this.ty[i] = t ? t.y : this.y[i]
        this.tz[i] = t ? t.z : this.z[i]
      }
    }
    ...
    for (const b of mol.bonds) {
      ...
      const len = lookupBondLengthByOrder(this.sym[i], this.sym[j], b.order)
                ?? lookupBondLengthByOrder(this.sym[i], this.sym[j], 1)!
    ...
    for (let c = 0; c < n; c++) {
      const deg = nbr[c].length
      // 度数 >4（八面体 / 超价）单一理想角不适用，跳过角约束，靠键 + 斥力自然展开
      if (deg < 2 || deg > 4) continue
      const hyb = inferHybridization(mol.bonds, this.ids[c])
      const geom = inferGeometry(this.sym[c], deg, hyb)
      const cosT = Math.cos(GEOMETRY_RULES[geom].bondAngle * DEG)
      for (let a = 0; a < deg; a++) {
        for (let b = a + 1; b < deg; b++) {
          const na = nbr[c][a], nb = nbr[c][b]
  ...
  private project(c: DistConstraint): void {
    const { i, j, d, k } = c
    let dx = this.x[j] - this.x[i], dy = this.y[j] - this.y[i], dz = this.z[j] - this.z[i]
    const dist = Math.sqrt(dx * dx + dy * dy + dz * dz) || 1e-6
    const res = dist - d
    if (Math.abs(res) > this._residual) this._residual = Math.abs(res)
    const f = (res / dist) * 0.5 * k
    dx *= f; dy *= f; dz *= f
    this.x[i] += dx; this.y[i] += dy; this.z[i] += dz
    this.x[j] -= dx; this.y[j] -= dy; this.z[j] -= dz
  }
  ...
  step(iterations = 1): void {
    for (let it = 0; it < iterations; it++) {
      ...
      if (this.tx && this.ty && this.tz) {
        ...
        for (let i = 0; i < N; i++) {
          this.x[i] += (this.tx[i] - this.x[i]) * ak
          this.y[i] += (this.ty[i] - this.y[i]) * ak
          this.z[i] += (this.tz[i] - this.z[i]) * ak
        }
      }
  ...
  positions(): Map<string, { x: number; y: number; z: number }> {
    ...
    for (let i = 0; i < n; i++) m.set(this.ids[i], { x: this.x[i] - cx, y: this.y[i] - cy, z: this.z[i] - cz })
    return m
  }

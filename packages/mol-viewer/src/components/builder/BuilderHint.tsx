...
const HINTS: Record<string, string> = {
  ...
  'select-armed':    '单击空白加原子 · 点 H 生长 · 拖 H 成键 · Esc 切换到选择',
...
export default function BuilderHint() {
  ...
  if (hint && flashVisible) {
    return (
      <div className="absolute bottom-[7rem] left-1/2 -translate-x-1/2 bg-gray-900/90 text-white text-xs px-4 py-1.5 rounded-full pointer-events-none shadow-lg">
        {hint.text}
      </div>
    )
  }
  ...
  return (
    <div className="absolute bottom-[7rem] left-1/2 -translate-x-1/2 flex items-center gap-1.5 bg-white/80 backdrop-blur-sm text-gray-400 text-[10px] px-2.5 py-1 rounded-full pointer-events-none border border-gray-200/80 shadow-sm">
      {sketchPlane && (
      ...
      <span>
        {toolCan(activeTool, 'canEdit')
          ? (brushArmed
              ? (fragment
                  ? '单击空白放置 · 点 H/原子接上 · 点键并环 · Esc 切换到选择'
                  : HINTS['select-armed'])

...
export const TEMPLATE_MOLECULES = SAMPLE_MOLECULES

export type TemplateMolecule = ReturnType<typeof SAMPLE_MOLECULES[number]['mol']>
export type SetMoleculeAction = (molecule: Molecule) => void
...
export function splitTemplateName(name: string) {
  const match = name.match(/^(.*?)\s*\((.+)\)\s*$/)
  return {
    main: match ? match[1] : name,
    sub: match ? match[2] : '',
  }
}

export function setCenteredMolecule(setMolecule: SetMoleculeAction, molecule: Molecule) {

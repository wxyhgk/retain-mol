  ...
  return (
    <InspectorLayout>
      <div className="flex min-w-0 items-center gap-2.5">
        ...
        <div className="min-w-0 flex-1"><div className="truncate text-sm font-semibold text-slate-900">{element.name}</div><div className="truncate text-[11px] text-slate-500">原子 #{number}</div></div>
      ...
      <InspectorSection title={`邻接键 · ${model.neighbors.length}`}>
        {model.neighbors.length === 0 ? <div className="rounded-md border border-dashed border-slate-200 px-2.5 py-3 text-center text-xs text-slate-500">无邻接键</div> : (
          <div className="divide-y divide-slate-200 overflow-hidden rounded-md border border-slate-200 bg-slate-50">{model.neighbors.map(neighbor => (
            <div key={neighbor.bond.id} className="flex min-w-0 items-center gap-2 px-2.5 py-2"><ElementSwatch symbol={neighbor.atom.symbol} /><span className="min-w-0 flex-1 truncate text-xs text-slate-700">{neighbor.atom.symbol} #{neighbor.atomNumber}</span><span className="shrink-0 font-mono text-[11px] tabular-nums text-slate-500">{neighbor.length.toFixed(3)} Å</span></div>
          ))}</div>
        )}
      </InspectorSection>
      <div className="grid min-w-0 grid-cols-2 gap-2 border-t border-slate-200 pt-3">
...
export function BondInspector({ model }: { model: BondModel }) {
  ...
  return (
    <InspectorLayout>
      <div className="flex min-w-0 items-center gap-2"><EndpointBadge atom={model.first.atom} number={model.first.number} /><span className="shrink-0 text-slate-600">—</span><EndpointBadge atom={model.second.atom} number={model.second.number} /></div>
      ...
      <InspectorSection title="键级"><div className="grid grid-cols-3 overflow-hidden rounded-md border border-slate-200 bg-slate-50 p-1">{([1, 2, 3] as const).map(order => (
      ...
      <div className="border-t border-slate-200 pt-3"><ActionButton danger icon={<Trash2 />} label="删除键" className="w-full" onClick={() => removeBond(model.bond.id)} /></div>
    </InspectorLayout>
  )
}
...
function EditableGeometryPanel({ geometry }: { geometry: EditableGeometry }) {
  ...
  return (
    <InspectorSection title={label}><div className="space-y-2 rounded-md border border-slate-200 bg-slate-50 p-2.5">
      <div className="flex min-w-0 flex-wrap items-center gap-1">{geometry.atoms.map(({ atom, number }, index) => <div key={atom.id} className="contents">{index > 0 && <span className="text-[11px] text-slate-600">—</span>}<EndpointBadge atom={atom} number={number} compact /></div>)}</div>

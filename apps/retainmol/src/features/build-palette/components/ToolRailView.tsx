...
import { Atom, CircleHelp, CirclePlus, Ellipsis, Eraser, Hexagon, Moon, Ruler, Settings, Sun, Type } from 'lucide-react'
...
import type { UiTheme } from '@/domain/uiThemeStore'
import { cn } from '@/lib/utils'
import type { RailItemCommand } from './toolRailCommands'

type RailItem = RailItemCommand & {
  label: string
  icon: ReactNode
}
...
type ToolRailViewProps = {
  workspaceTool: WorkspaceTool
  uiTheme: UiTheme
  activate: (item: RailItemCommand) => void
  onToggleInspector: () => void
  showHelp: () => void
  toggleTheme: () => void
}

export function ToolRailView({ workspaceTool, uiTheme, activate, onToggleInspector, showHelp, toggleTheme }: ToolRailViewProps) {
  return (
    <Sidebar collapsible="none" className="w-[72px] border-r border-border bg-sidebar text-sidebar-foreground">
      <SidebarContent className="gap-0 overflow-y-auto px-1.5 py-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        <SidebarGroup className="p-0">
          <SidebarMenu className="gap-0.5">
            {ITEMS.map(item => {
              const active = Boolean(item.workspaceTool && workspaceTool === item.workspaceTool)
              return (
      ...
      <SidebarFooter className="items-center gap-1 px-1.5 py-2">
        ...
        <SidebarMenu className="items-center gap-1">
          <Utility label="Help" onClick={showHelp}><CircleHelp /></Utility>

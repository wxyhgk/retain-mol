...
exports.buildCanvasCandidate = buildCanvasCandidate;
const ketcher_core_1 = require("ketcher-core");
...
const bondTypes = { single: 1, double: 2, triple: 3 };
function failure(code, message, references) {
    return {
        ok: false,
        error: { code, message, ...(references ? { references } : {}) },
    };
}
function graphKey(document) {
...
function emptyPool(source, reserved = []) {
...
function copyDisplayData(value, ancestors = new Set()) {
    ...
    if (ancestors.has(value))
        throw new Error('Cyclic display metadata');
    const prototype = Object.getPrototypeOf(value);
    if (![
        Object.prototype,
        Array.prototype,
        ketcher_core_1.Highlight.prototype,
        ketcher_core_1.Vec2.prototype,
        null,
    ].includes(prototype)) {
...
function oldFragments(source) {
    ...
    for (const start of source.atoms.keys()) {
        ...
        if (source.frags.get(fragmentId) &&
...
function buildCanvasCandidate(source, base, candidate, identities) {
    try {
        const checkedBase = (0, molecule_contracts_1.validateDocumentSnapshot)(base);
        const checkedCandidate = (0, molecule_contracts_1.validateDocumentSnapshot)(candidate);
        if (!checkedBase.ok || !checkedCandidate.ok) {
            return failure('invalid-structure', 'Expected valid base and candidate molecule documents.');
        }
        base = checkedBase.value;
        candidate = checkedCandidate.value;
        if (candidate.documentId !== base.documentId ||
            candidate.revision !== base.revision) {
            return failure('invalid-structure', 'The candidate must belong to the prepared base document and revision.');
        }
        const missingCoordinates = candidate.atoms
            .filter((atom) => !atom.position)
            .map((atom) => atom.id);
        if (missingCoordinates.length)
            return failure('missing-coordinates', 'Every candidate atom requires an explicit 2D position.', missingCoordinates);
        if (candidate.atoms.length > 10_000 || candidate.bonds.length > 20_000) {
            return failure('limit-exceeded', 'The candidate exceeds basic graph limits.');
        }
        const knownIds = new Set();
        const atomSlots = new Map();
        const bondSlots = new Map();
        for (const [map, reverse, max] of [
            ...
            for (const [slot, id] of map) {
                if (!Number.isSafeInteger(slot) || slot < 0 || slot > max) {
                    return failure('invalid-structure', 'Canvas pool slots exceed the safe runtime ID range.');
                }
                if (typeof id !== 'string' || !id || knownIds.has(id)) {
                    return failure('invalid-structure', 'Canvas entity identities must be distinct nonempty strings.');
                }
                knownIds.add(id);
                reverse.set(id, slot);
            }
        }
        const projected = (0, projection_js_1.projectStructure)(source, {
            documentId: base.documentId,
            revision: base.revision,
            atomId: (slot) => {
                const id = identities.atoms.get(slot);
                if (!id)
                    throw new Error('Unmapped canvas atom');
                return id;
            },
            bondId: (slot) => {
                const id = identities.bonds.get(slot);
                if (!id)
                    throw new Error('Unmapped canvas bond');
                return id;
            },
        });
        if (!projected.ok || graphKey(projected.document) !== graphKey(base)) {
            return failure('canvas-changed', 'The live canvas no longer matches the prepared base graph.');
        }
        ...
        for (const [entities, existing] of [
            ...
            for (const entity of entities) {
                if (candidateIds.has(entity.id) ||
                    ...
                    return failure('invalid-structure', 'A candidate cannot duplicate or reuse an entity identity.', [entity.id]);
                }
                candidateIds.add(entity.id);
            }
        }
        const struct = new ketcher_core_1.Struct();
        ...
        for (const value of candidate.atoms) {
            ...
            const previous = previousSlot === undefined ? undefined : source.atoms.get(previousSlot);
            const atom = previous
                ? new ketcher_core_1.Atom(previous)
                : new ketcher_core_1.Atom({ label: value.element });
            atom.label = value.element;
            if (!previous || (previous.charge ?? 0) !== value.charge)
                atom.charge = value.charge;
            const previousIsotope = previous?.isotope == null || previous.isotope === 0
                ? undefined
                : previous.isotope;
            if (!previous || previousIsotope !== value.isotope)
                atom.isotope = value.isotope ?? null;
            atom.pp = new ketcher_core_1.Vec2(value.position);
            if (previous) {
        ...
        for (const value of candidate.bonds) {
            if (!currentAtoms.has(value.begin) ||
                !currentAtoms.has(value.end) ||
                value.begin === value.end) {
                return failure('invalid-structure', 'A candidate bond must join two existing, distinct atoms.', [value.id]);
            }
            const edge = JSON.stringify([value.begin, value.end].sort());
            if (edges.has(edge))
                return failure('invalid-structure', 'Only one bond may join an atom pair.', [value.id]);
            edges.add(edge);
            ...
            const previous = previousSlot === undefined ? undefined : source.bonds.get(previousSlot);
            const bond = new ketcher_core_1.Bond({
                ...(previous ?? {}),
        ...
        for (const component of struct.findConnectedComponents(true)) {
            ...
            const previous = previousSlot === undefined ? undefined : source.frags.get(previousSlot);
            const slot = previousSlot ?? struct.frags.newId();
            if (slot > MAX_POOL_SLOT)
                return failure('limit-exceeded', 'No safe fragment pool slot remains.');
            const fragment = new ketcher_core_1.Fragment([], [...component].every((id) => source.atoms.get(id)?.pp.x === struct.atoms.get(id).pp.x &&
                source.atoms.get(id)?.pp.y === struct.atoms.get(id).pp.y)
                ? previous?.stereoFlagPosition
                : undefined, previous?.properties?.map((property) => ({ ...property })));
            struct.frags.set(slot, fragment);
            for (const atomId of component)
                struct.atoms.get(atomId).fragment = slot;
        }
        struct.findLoops();
        const finalProjection = (0, projection_js_1.projectStructure)(struct, {
        ...
        if (!finalProjection.ok ||
            ...
            return failure('invalid-structure', 'The isolated canvas candidate does not match its prepared graph.');
        }
        return { ok: true, value: { struct, identities: mapped } };
    }
    catch {
        return failure('invalid-structure', 'The canvas candidate could not be materialized safely.');
    }
}

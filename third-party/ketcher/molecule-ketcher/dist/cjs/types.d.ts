...
import type { MoleculeCanvasChangeReason, MoleculeCanvasIssue, MoleculeCanvasApi } from 'molecule-contracts';
/** Host-owned committed-change signals, never renderer or pointer events. */
export interface MoleculeCanvasSource {
    getStruct(): Struct;
    subscribe(listener: (reason: Exclude<MoleculeCanvasChangeReason, 'initial'>) => void): () => void;
...
export interface MoleculeCanvasEditSource extends MoleculeCanvasSource {
     ...
     */
    commit(struct: Struct, expected: Struct, onCommitted: () => void): MoleculeCanvasCommitResult;
}
export interface MoleculeCanvasEditorOptions extends MoleculeCanvasReaderOptions {

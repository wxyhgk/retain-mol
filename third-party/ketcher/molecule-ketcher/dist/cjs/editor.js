...
exports.createMoleculeCanvasEditor = createMoleculeCanvasEditor;
const molecule_contracts_1 = require("molecule-contracts");
const molecule_engine_1 = require("molecule-engine");
const candidate_js_1 = require("./candidate.js");
...
function failure(code, message, currentRevision) {
    return freeze({
        ok: false,
        error: {
            code,
...
function createMoleculeCanvasEditor(source, options = {}) {
    if (!source ||
        typeof source.getEditBusyReason !== 'function' ||
        typeof source.commit !== 'function') {
        return failure('invalid-request', 'Atomic canvas editing requires busy and commit ports.');
    }
    ...
    for (const limit of [maxPreparedEdits, maxIdempotencyEntries]) {
        if (!Number.isSafeInteger(limit) || limit < 1 || limit > 10_000) {
            return failure('invalid-request', 'Canvas cache limits must be integers between 1 and 10000.');
        }
    }
    const result = (0, reader_js_1.createCanvasReaderController)(source, options);
    if (!result.ok)
        return result;
    ...
    if (reader.getState().status === 'disposed')
        release();
    ...
    function checkVersion(base) {
        const current = reader.getDocument();
        if (!current.ok)
            return current;
        if (base.documentId !== current.value.documentId)
            return failure('document-mismatch', 'The edit belongs to a different canvas.');
        if (base.revision !== current.value.revision)
            return failure('revision-conflict', 'The canvas changed after preparation. Prepare a new edit.', current.value.revision);
        if (current.value.revision === Number.MAX_SAFE_INTEGER)
            return failure('limit-exceeded', 'The canvas revision limit was reached.');
        return current;
    }
    function disposed() {
    ...
    function prepareEdit(input) {
        ...
        if (existing)
            return existing.fingerprint === fingerprint
                ...
                : failure('request-id-conflict', 'The request ID is already bound to different edit content.');
        const current = reader.getDocument();
        if (!current.ok)
            return current;
        ...
        if (request.baseRevision !== current.value.revision)
            return failure('revision-conflict', 'The edit base revision is stale.', current.value.revision);
        if (pendingById.size >= maxPreparedEdits)
            return failure('limit-exceeded', 'Commit or cancel a pending edit before preparing another.');
        const session = (0, molecule_engine_1.createMoleculeSession)({
        ...
        if (prepared.value.candidate.atoms.some((atom) => !atom.position)) {
            return failure('missing-coordinates', 'Every canvas atom requires an explicit position; automatic layout is unavailable.');
        }
        const entry = {
            base: current.value,
    ...
    function commitEdit(input) {
        ...
        if (completed)
            return completed.prepared.requestId === request.requestId
                ...
                : failure('request-id-conflict', 'The prepared edit belongs to a different request ID.');
        if (committing)
            return failure('canvas-busy', 'An atomic canvas commit is in progress.');
        const pending = pendingById.get(request.preparedId);
        if (!pending)
            return failure('prepared-not-found', 'The prepared edit is absent, cancelled, evicted or from another canvas.');
        if (pending.prepared.requestId !== request.requestId)
            return failure('request-id-conflict', 'The prepared edit belongs to a different request ID.');
        ...
        try {
            ...
            if (busy)
                return failure('canvas-busy', busy);
            const live = host.getStruct();
            const identities = controller.getIdentities(live);
            if (!identities)
                return failure('canvas-changed', 'The live canvas differs from the committed snapshot.');
            const built = (0, candidate_js_1.buildCanvasCandidate)(live, pending.base, pending.prepared.candidate, identities);
            if (!built.ok)
                return freeze(built);
            ...
            if (committed.ok)
                return failure('canvas-commit-failed', 'The host did not acknowledge the atomic commit.');
            const codes = {
                busy: 'canvas-busy',
    ...
    function cancelEdit(input) {
        ...
        const pending = pendingById.get(request.preparedId);
        if (!pending)
            return failure('prepared-not-found', 'There is no pending edit to cancel.');
        if (pending.prepared.requestId !== request.requestId)
            return failure('request-id-conflict', 'The prepared edit belongs to a different request ID.');
        pendingById.delete(request.preparedId);
        pendingByRequest.delete(request.requestId);

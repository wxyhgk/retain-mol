...
function copyJson(input: unknown): Result<Json> {
  const ancestors = new Set<object>();
  let visited = 0;
  function visit(value: unknown, parts: (string | number)[]): Result<Json> {
    if (++visited > MAX_JSON_NODES || parts.length > MAX_JSON_DEPTH) {
      return {
        ok: false,
        error: failure(
          'JSON input exceeds the validation size or depth limit.',
          parts,
          true,
        ),
      };
    ...
    if (keys.length > MAX_JSON_NODES - visited) {
      return {
        ok: false,
        error: failure(
          'JSON input exceeds the validation size limit.',
          parts,
          true,
        ),
      };
    ...
    if (
      ...
      return {
        ok: false,
        error: failure(
          'Expected a dense JSON array without extra properties.',
          parts,
        ),
      };
    }
...
function schemaError(
  ...
  const alternatives = schema?.oneOf?.map((branch) =>
  ...
  if (
    ...
    return failure(
      'Expected exactly one reference: { id: "existing-id" } or { ref: "request-ref" }; its value must be a nonempty string of at most 128 characters.',
      [...prefix, ...error.path],
    );
  }
  if (
    ...
    if (branch) {
      if (
        ...
        return failure(
          `Expected an integer from ${branch.minimum} to ${branch.maximum}, or null to clear the field.`,
          [...prefix, ...error.path],
        );
      }
      const detail = validator.validate(error.instance, branch).errors[0];
      if (detail) return schemaError(detail, [...prefix, ...error.path]);
    }
  }
  return failure(error.message, [...prefix, ...error.path]);
}

function validate<T>(input: unknown, schema: Schema): Result<T> {
  const copied = copyJson(input);
  if (!copied.ok) return copied;
  try {
    const result = validator.validate(copied.value, schema);
    if (result.valid) return { ok: true, value: copied.value as T };
    ...
    if (
      schema === moleculeEditRequestSchema &&
      error.path[0] === 'commands' &&
      typeof error.path[1] === 'number'
    ) {
      const input = copied.value as unknown as MoleculeEditRequest;
      const command = input.commands[error.path[1]];
      const itemSchema = schema.properties?.commands.items as Schema;
      const branch = itemSchema.oneOf?.find(
        (item) => item.properties?.op.const === command?.op,
      );
      if (branch) {
      ...
      if (!branch) {
        return {
          ok: false,
          error: failure(
            `Expected a command with one of these operations: ${itemSchema.oneOf
    ...
    return {
      ok: false,
      error: failure(
        'JSON input could not be validated against this protocol.',
        [],
      ),
    };
  }
...
export function validateCommitRequest(input: unknown): Result<CommitRequest> {
  return validate(input, moleculeCommitRequestSchema);
}

export function validateCancelRequest(input: unknown): Result<CancelRequest> {
  return validate(input, moleculeCommitRequestSchema);
}

export function validateHistoryRequest(input: unknown): Result<HistoryRequest> {

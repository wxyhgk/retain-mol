...
import { FragmentAddStereoAtom } from 'application/editor/operations';
...
describe('fromBondFlipping', () => {
  ...
  it('rolls back stereo attributes when fragment membership update fails', () => {
    const { restruct, beginId } = buildRestruct(Bond.PATTERN.STEREO.NONE);
    jest
      .spyOn(FragmentAddStereoAtom.prototype, 'perform')
      .mockImplementationOnce(() => {
        throw new Error('fragment stereo update failed');
      });

    expect(() =>
      bondStereoModule.fromStereoAtomAttrs(
        restruct as never,
        beginId,
        { stereoLabel: 'abs', stereoParity: 1 },
        false,
      ),
    ).toThrow('fragment stereo update failed');

    expect(restruct.molecule.atoms.get(beginId)?.stereoLabel).toBeNull();
    expect(restruct.molecule.atoms.get(beginId)?.stereoParity).toBe(0);
    expect(restruct.molecule.frags.get(0)?.stereoAtoms).toEqual([]);
  });

  it('rolls back the entire bond flip when the final stereo step fails', () => {
    const { restruct, bondId, beginId, endId } = buildRestruct(
      Bond.PATTERN.STEREO.UP,
    );
    jest
      .spyOn(FragmentAddStereoAtom.prototype, 'perform')
      .mockImplementationOnce(() => {
        throw new Error('final stereo step failed');
      });

    expect(() => fromBondFlipping(restruct as never, bondId)).toThrow(
      'final stereo step failed',
    );

    expect(
      Array.from(restruct.molecule.bonds.values()).some(
        (bond) => bond.begin === beginId && bond.end === endId,
      ),
    ).toBe(true);
    expect(restruct.molecule.atoms.get(beginId)?.stereoLabel).toBe('abs');
    expect(restruct.molecule.atoms.get(beginId)?.stereoParity).toBe(1);
    expect(restruct.molecule.atoms.get(endId)?.stereoLabel).toBeNull();
    expect(restruct.molecule.frags.get(0)?.stereoAtoms).toEqual([beginId]);
  });
});

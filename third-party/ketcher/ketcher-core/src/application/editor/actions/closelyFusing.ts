...
export function fromItemsFuse(restruct, items) {
  ...
  try {
    ...
    // merge bonds
    const bondsMergeAction = fromBondsMerge(restruct, items.bonds);
    transaction.capture(bondsMergeAction);
    action = bondsMergeAction.mergeWith(action);

    const valenceAction = valenceCheck(restruct, connectedAtomIds);
...
export function getItemsToFuse(
  struct: Struct,
  mergeCandidates: FuseMergeCandidates,
): ItemsToFuse | null {
...
function closestToMerge(
  struct: Struct,
  closestMap: FuseMergeCandidates,
): ItemsToFuse | null {
  const mergeMap = {

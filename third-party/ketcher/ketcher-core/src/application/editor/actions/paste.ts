...
export function fromPaste(
  ...
  try {
    pstruct.atoms.forEach((atom, aid) => {
      ...
      const attachmentPointAction = fromRGroupAttachmentPointAddition(
        restruct,
    ...
    pasteItems.atoms.forEach((aid) => {
      action.addOp(
        transaction.capture(new CalcImplicitH([aid]).perform(restruct)),
      );
      transaction.capture(
    ...
    pstruct.rxnPluses.forEach((plus) => {
      const operation = transaction.capture(
        new RxnPlusAdd(plus.pp.add(offset)).perform(restruct),
      );
      action.addOp(operation);
    ...
    pstruct.images.forEach((image: Image) => {
      ...
      const operation = transaction.capture(
        new ImageUpsert(clonedImage).perform(restruct),
      );
      action.addOp(operation);
    ...
    pstruct.rgroups.forEach((rg, rgid) => {
      rg.frags.forEach((__frag, frid) => {
        ...
        action.addOp(
          transaction.capture(
            new RGroupFragment(rgid, newFragmentId).perform(restruct),
          ),
        );
      });
      const ifThen = rg.ifthen;
      const safeIfThenId = pstruct.rgroups.get(ifThen) ? ifThen : 0;
      const attrsAction = fromRGroupAttrs(restruct, rgid, rg.getAttrs());
      transaction.capture(attrsAction);
      action.mergeWith(attrsAction);
      const ifThenAction = fromUpdateIfThen(restruct, safeIfThenId, rg.ifthen);

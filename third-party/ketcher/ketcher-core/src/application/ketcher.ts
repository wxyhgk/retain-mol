...
import {
  ...
  type MoleculeEditPlanPlaybackOptions,
...
} from 'application/formatters/computationalFormatMetadata';
import {
...
export class Ketcher {
  ...
  #moleculeReader: MoleculeCanvasApi | null = null;
  #disposeMoleculeReader?: () => void;
  #moleculeCanvasUnavailableReason: string | null = null;
  _indigo: Indigo;
  #recognitionAdapter?: MoleculeRecognitionAdapter;
  ...
  get molecule(): MoleculeCanvasApi | null {
    return this.#moleculeReader;
  }

  get moleculeCanvasUnavailableReason(): string | null {
    return this.#moleculeCanvasUnavailableReason;
  }
  ...
  setMoleculeCanvasUnavailableReason(reason: string | null): void {
    if (reason === this.#moleculeCanvasUnavailableReason) return;
    this.#moleculeCanvasUnavailableReason = reason;
    this.#editor?.notifyDocumentChange?.('mode');
  }

  /** Host injection keeps Ketcher independent of the canvas adapter package. */
  setMoleculeReader(
    reader: MoleculeCanvasApi | null,
    dispose?: () => void,
  ): void {
    if (reader === this.#moleculeReader) return;
    const previousDispose = this.#disposeMoleculeReader;
    this.#moleculeReader = reader;
    this.#disposeMoleculeReader = dispose;
    previousDispose?.();
  }
  ...
  addEditor(editor: Editor) {
    if (this.#editor !== editor) this.setMoleculeReader(null);
    this.#editor = editor;
  }
  ...
  setRecognitionAdapter(adapter?: MoleculeRecognitionAdapter | null): void {
    this.#recognitionAdapter = adapter ?? undefined;
  }
  ...
  async setMolecule(
    ...
    await runAsyncAction<void>(async () => {
      ...
      if (window.isPolymerEditorTurnedOn) {
        ...
        const sourceFormat = identifyStructFormat(structStr);
        const preservesCalculationGeometry =
          isCalculationGeometryFormat(sourceFormat);
  ...
  getCalculationSnapshot(
    ...
    const importedMetadata = getComputationalFormatMetadata(struct);
    const snapshotOptions: CreateCalculationSnapshotV1Options = {
      ...options,
      totalCharge: options?.totalCharge ?? importedMetadata?.molecularCharge,
      multiplicity:
        options?.multiplicity === undefined
          ? importedMetadata?.molecularMultiplicity
  ...
  setCalculationSettings(settings: {
    ...
    this.editor.notifyDocumentChange?.('untracked');
  }
  ...
  recognize(image: Blob, version?: string): Promise<Struct> {
    ...
    if (this.#recognitionAdapter) {
      return this.#recognitionAdapter
  ...
  async playMoleculeEditPlan(
    plan: MoleculeEditPlanV1,
    options: MoleculeEditPlanPlaybackOptions = {},
  ): Promise<MoleculeEditPlanExecutor> {
    if (window.isPolymerEditorTurnedOn) {
      throw new Error(
        'Molecule edit plan playback is not available in macro mode',
      );
    }
    if (this.editor.render.options.viewOnlyMode) {
      throw new Error(
        'Molecule edit plan playback is not available in view-only mode',
      );
    }
    const executor = new MoleculeEditPlanExecutor(
      this.editor,
      plan,
      options.positionOffset,
    );
    await executor.play(options);
    return executor;
  }

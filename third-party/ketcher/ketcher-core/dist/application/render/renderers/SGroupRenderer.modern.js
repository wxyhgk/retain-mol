...
import { SUPERATOM_CLASS_TEXT } from '../restruct/resgroup.modern.js';
...
var DEFAULT_PADDING_VECTOR = new Vec2(0.2, 0.4);
var COP_PADDING_VECTOR = new Vec2(1.2, 1.2);
...
var SGroupRenderer = function (_BaseRenderer) {
  _inherits(SGroupRenderer, _BaseRenderer);
  function SGroupRenderer(sgroupDrawingEntity) {
    var _this;
    _classCallCheck(this, SGroupRenderer);
    _this = _callSuper(this, SGroupRenderer, [sgroupDrawingEntity]);
    _defineProperty(_assertThisInitialized(_this), "sgroupDrawingEntity", void 0);
    _defineProperty(_assertThisInitialized(_this), "labelElements", []);
    _defineProperty(_assertThisInitialized(_this), "atomRenderers", new Map());
    _defineProperty(_assertThisInitialized(_this), "bondRenderers", new Map());
    _this.sgroupDrawingEntity = sgroupDrawingEntity;
    _this.sgroupDrawingEntity.setRenderer(_assertThisInitialized(_this));
    return _this;
  }
  _createClass(SGroupRenderer, [{
    key: "struct",
    get: function get() {
    ...
    value: function getSuperatomLabel() {
      return this.sgroup.data.name || SUPERATOM_CLASS_TEXT[this.sgroup.data["class"]];
    }
  }, {
    ...
    value: function drawDataSGroup() {
      ...
      if (this.sgroup.data.attached) {
        SGroup.getAtoms(this.struct, this.sgroup).forEach(function (atomId) {
          var atom = _this3.struct.atoms.get(atomId);
          if (!atom) {
            return;
          }
          var position = Scale.modelToCanvas(atom.pp, _this3.editorSettings).add(new Vec2(_this3.editorSettings.microModeScale / LINE_WIDTH_SCALE_BASE, 0));
      ...
      if (this.sgroup.pp) {
        var position = Scale.modelToCanvas(this.sgroup.pp, this.editorSettings);
        this.appendValue(position, function (bbox) {
          return _this3.sgroup.data.context === SgContexts.Bond ? _this3.getCenteredDataSGroupLabelShift(position, bbox) : _this3.getAbsoluteDataSGroupLabelShift(position, bbox);
        });
      }
    }
  }, {
    ...
    value: function calculateBracketBox() {
      ...
      SGroup.getAtoms(this.struct, this.sgroup).forEach(function (atomId) {
        ...
        contentBoxes.push(new Box2Abs(atom.pp, atom.pp).extend(BORDER_EXT, BORDER_EXT));
      });
      var bracketBox = contentBoxes.reduce(function (box, contentBox) {
        return box ? Box2Abs.union(box, contentBox) : contentBox;
      }, null);
      if (!bracketBox) {
        return null;
      }
      var paddingVector = SGroup.isCOPGroup(this.sgroup) ? COP_PADDING_VECTOR : DEFAULT_PADDING_VECTOR;
    ...
    value: function getBracketParameters(bracketBox) {
      ...
      var leftCenter = Vec2.lc2(angleDirection, bracketBox.p0.x, bracketDirection, 0.5 * (bracketBox.p0.y + bracketBox.p1.y));
      var rightCenter = Vec2.lc2(angleDirection, bracketBox.p1.x, bracketDirection, 0.5 * (bracketBox.p0.y + bracketBox.p1.y));
    ...
    value: function appendBracket(_ref2) {
      ...
      var center = _ref2.center,
        ...
        width = _ref2.width,
        height = _ref2.height;
      var scaledCenter = Scale.modelToCanvas(center, this.editorSettings);
      var scaledDirection = Scale.modelToCanvas(direction, this.editorSettings);
      var scaledAngleDirection = Scale.modelToCanvas(angleDirection, this.editorSettings);
    ...
    value: function appendIndex(text, bracket, isLowerText, indexAttribute) {
      ...
      var position = Scale.modelToCanvas(bracketEdge.addScaled(bracket.angleDirection, 0.05), this.editorSettings).add(new Vec2(0, INDEX_LABEL_VERTICAL_SHIFT));
    ...
    value: function getCenteredDataSGroupLabelShift(position, bbox) {
      return new Vec2(position.x - bbox.x - bbox.width / 2, position.y - bbox.y - bbox.height / 2);
    }
  }, {
    ...
    value: function getAttachedDataSGroupLabelShift(position, bbox) {
      return new Vec2(position.x - bbox.x, position.y - bbox.y - ATTACHED_LABEL_VERTICAL_SHIFT_FACTOR * bbox.height);
    }
  }, {
    key: "appendValue",
    value: function appendValue(scaledPosition, getLabelShift) {
      ...
      var valueBackgroundColor = this.sgroup.selected ? SELECTION_COLOR : DATA_SGROUP_BACKGROUND;
    ...
    value: function remove() {
      _get(_getPrototypeOf(SGroupRenderer.prototype), "remove", this).call(this);
      this.labelElements.forEach(function (labelElement) {
        labelElement.remove();
      });
      this.labelElements = [];
    ...
    value: function setVisibility(isVisible) {
      _get(_getPrototypeOf(SGroupRenderer.prototype), "setVisibility", this).call(this, isVisible);
      this.labelElements.forEach(function (labelElement) {
    ...
    value: function getScaledBracketBox() {
      ...
      return (_this$sgroup$bracketB = this.sgroup.bracketBox) === null || _this$sgroup$bracketB === void 0 ? void 0 : _this$sgroup$bracketB.transform(Scale.modelToCanvas, this.editorSettings);
    }
  }, {
    ...
    value: function getSGroupAtomRenderers() {
      var _this6 = this;
      var sgroupAtomIds = this.getSGroupAtomIds();
      return _toConsumableArray(this.atomRenderers.values()).filter(function (atomRenderer) {
    ...
    value: function getSGroupBondRenderers() {
      var _this7 = this;
      var sgroupAtomIds = this.getSGroupAtomIds();
      return _toConsumableArray(this.bondRenderers.values()).filter(function (bondRenderer) {
    ...
    value: function getAtomHoverPath(atomRenderer) {
      ...
      if (contour.type === 'circle') {
        return new paperjs.Path.Circle(new paperjs.Point(contour.center.x, contour.center.y), contour.radius);
      }
      return new paperjs.Path.Rectangle(new paperjs.Rectangle(contour.x, contour.y, contour.width, contour.height), new paperjs.Size(contour.radius, contour.radius));
    }
  }, {
    ...
    value: function getCombinedStructureHoverPathData() {
      ...
      var hoverPaths = [].concat(_toConsumableArray(this.getSGroupAtomRenderers().map(function (atomRenderer) {
        return _this8.getAtomHoverPath(atomRenderer);
      })), _toConsumableArray(this.getSGroupBondRenderers().map(function (bondRenderer) {
...
export { SGroupRenderer };
//# sourceMappingURL=SGroupRenderer.modern.js.map

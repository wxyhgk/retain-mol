...
import { createSegmentHighlightPath } from '../monomerHighlightShapes.modern.js';
...
var ReplacementHighlightView = function (_TransientView) {
  _inherits(ReplacementHighlightView, _TransientView);
  function ReplacementHighlightView() {
    _classCallCheck(this, ReplacementHighlightView);
    return _callSuper(this, ReplacementHighlightView, arguments);
  }
  _createClass(ReplacementHighlightView, null, [{
    key: "collectPathData",
    value:
      ...
      try {
        for (_iterator.s(); !(_step = _iterator.n()).done;) {
          ...
          for (var _i = 0, _Object$values = Object.values(monomer.attachmentPointsToBonds); _i < _Object$values.length; _i++) {
            ...
            pathData.push(createSegmentHighlightPath(from, to, NECK_HALF_WIDTH + OUTLINE_GAP));
          }
        }
      } catch (err) {
        _iterator.e(err);
    ...
    value: function getUnitedPathData(pathsData) {
      ...
      pathsData.forEach(function (pathData) {
        var path = new paperjs.CompoundPath(pathData);
        if (!path.closed) {
          path.closePath();
        }
        if (!combinedPath) {
    ...
    value: function show(transientLayer, params) {
      var monomers = params.monomers.filter(function (monomer) {
        return monomer.renderer instanceof BaseMonomerRenderer;
      });
...
_defineProperty(ReplacementHighlightView, "viewName", 'ReplacementHighlightView');

export { ReplacementHighlightView };
//# sourceMappingURL=ReplacementHighlightView.modern.js.map

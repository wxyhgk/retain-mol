...
var PolymerBondRendererFactory = require('./PolymerBondRenderer/PolymerBondRendererFactory.js');
...
var AmbiguousMonomerRenderer = require('./AmbiguousMonomerRenderer.js');
var CoreAtom = require('../../../domain/entities/CoreAtom.js');
var AtomRenderer = require('./AtomRenderer.js');
var BondRenderer = require('./BondRenderer.js');
var CoreBond = require('../../../domain/entities/CoreBond.js');
var MonomerToAtomBondRenderer = require('./MonomerToAtomBondRenderer.js');
var MonomerToAtomBond = require('../../../domain/entities/MonomerToAtomBond.js');
var MonomerToAtomBondSequenceRenderer = require('./sequence/MonomerToAtomBondSequenceRenderer.js');
var SequenceRenderer = require('./sequence/SequenceRenderer.js');
var PeptideSubChain = require('../../../domain/entities/monomer-chains/PeptideSubChain.js');
var RnaSubChain = require('../../../domain/entities/monomer-chains/RnaSubChain.js');
var PhosphateSubChain = require('../../../domain/entities/monomer-chains/PhosphateSubChain.js');
var RxnArrowRenderer = require('./RxnArrowRenderer.js');
var MultitailArrowRenderer = require('./MultitailArrowRenderer.js');
var RxnPlusRenderer = require('./RxnPlusRenderer.js');
var StereoFlagRenderer = require('./StereoFlagRenderer.js');
...
var SGroupRenderer = require('./SGroupRenderer.js');
...
var RenderersManager = function () {
  ...
  _createClass__default["default"](RenderersManager, [{
    ...
    value: function hoverDrawingEntity(drawingEntity) {
      assert.assert(drawingEntity.baseRenderer);
    ...
    value: function moveDrawingEntity(drawingEntity) {
      ...
      if (drawingEntity instanceof CoreAtom.Atom) {
    ...
    value: function addMonomer(monomer, callback) {
      ...
      if (monomer instanceof AmbiguousMonomer.AmbiguousMonomer) {
        monomerRenderer = new AmbiguousMonomerRenderer.AmbiguousMonomerRenderer(monomer);
      } else {
        var MonomerRenderer = monomerFactory.monomerFactory(monomer.monomerItem)[1];
    ...
    value: function addPolymerBond(polymerBond) {
      ...
      var polymerBondRenderer = PolymerBondRendererFactory.PolymerBondRendererFactory.createInstance(polymerBond);
    ...
    value: function isRnaEnumerationNode(node) {
      return node instanceof Nucleotide.Nucleotide || node instanceof Nucleoside.Nucleoside || node.monomer instanceof UnsplitNucleotide.UnsplitNucleotide;
    }
  }, {
    ...
    value: function recalculateRnaChainEnumeration(subChain, isChainCyclic) {
      ...
      if (isChainCyclic && !monomers.isValidRnaEnumerationStartMonomer(startMonomer)) {
        subChain.nodes.forEach(function (node) {
      ...
      subChain.nodes.forEach(function (node, nodeIndex) {
        ...
        if (node instanceof Nucleotide.Nucleotide || node instanceof Nucleoside.Nucleoside) {
          ...
          currentEnumeration++;
        } else if (node.monomer instanceof UnsplitNucleotide.UnsplitNucleotide) {
          ...
          currentEnumeration++;
        } else if (node instanceof MonomerSequenceNode.MonomerSequenceNode || node instanceof LinkerSequenceNode.LinkerSequenceNode) {
          node.monomers.forEach(function (monomer) {
    ...
    value: function recalculateMonomersEnumeration() {
      ...
      chainsCollection.chains.forEach(function (chain) {
        chain.subChains.forEach(function (subChain) {
          if (subChain instanceof PeptideSubChain.PeptideSubChain) {
            _this2.recalculatePeptideChainEnumeration(subChain);
          } else if (subChain instanceof RnaSubChain.RnaSubChain || subChain instanceof PhosphateSubChain.PhosphateSubChain) {
    ...
    value: function reinitializeViewModel() {
      var _this$editor;
      var editor = (_this$editor = this.editor) !== null && _this$editor !== void 0 ? _this$editor : editorSingleton.provideEditorInstance();
      var viewModel = editor.viewModel;
      viewModel.initialize(_toConsumableArray__default["default"](editor.drawingEntitiesManager.bonds.values()));
    }
  }, {
    key: "update",
    value: function update(modelChanges) {
      if (this.zoomTool) Zoom.ZoomTool.setRenderingContext(this.zoomTool);
      if (this.editor) editorSingleton.setEditorRenderingContext(this.editor);
      try {
        ...
        if (this.zoomTool) Zoom.ZoomTool.setRenderingContext(undefined);
        if (this.editor) editorSingleton.setEditorRenderingContext(undefined);
      }
    }
  }, {
    key: "addAtom",
    value: function addAtom(atom) {
      ...
      var atomRenderer = new AtomRenderer.AtomRenderer(atom);
    ...
    value: function addBond(bond) {
      ...
      var bondRenderer = new BondRenderer.BondRenderer(bond);
      ...
      this.bonds.forEach(function (redrawBondRenderer) {
        ...
        var monomerToAtomBonds = [].concat(_toConsumableArray__default["default"](firstAtom.bonds.filter(function (bond) {
          return bond instanceof MonomerToAtomBond.MonomerToAtomBond;
        })), _toConsumableArray__default["default"](secondAtom.bonds.filter(function (bond) {
          return bond instanceof MonomerToAtomBond.MonomerToAtomBond;
        })));
    ...
    value: function addSGroup(sgroupDrawingEntity) {
      ...
      var sgroupRenderer = new SGroupRenderer.SGroupRenderer(sgroupDrawingEntity);
    ...
    value: function addMonomerToAtomBond(bond) {
      ...
      if (sequenceNode) {
        var _renderer = new MonomerToAtomBondSequenceRenderer.MonomerToAtomBondSequenceRenderer(bond, sequenceNode);
        SequenceRenderer.SequenceRenderer.showBondRenderer(_renderer);
        this.redrawMonomerToAtomBondRelatedState(bond);
        return;
      }
      var renderer = new MonomerToAtomBondRenderer.MonomerToAtomBondRenderer(bond);
      renderer.show();
      this.redrawMonomerToAtomBondRelatedState(bond);
    }
  }, {
    ...
    value: function getSequenceNodeForMonomerToAtomBond(bond) {
      ...
      return (_SequenceRenderer$cha = SequenceRenderer.SequenceRenderer.chainsCollection) === null || _SequenceRenderer$cha === void 0 ? void 0 : _SequenceRenderer$cha.monomerToNode.get(bond.monomer);
    }
  }, {
    ...
    value: function addRxnArrow(arrow) {
      var arrowRenderer = new RxnArrowRenderer.RxnArrowRenderer(arrow);
      arrowRenderer.show();
    }
  }, {
    key: "deleteRxnArrow",
    ...
    value: function addMultitailArrow(arrow) {
      var arrowRenderer = new MultitailArrowRenderer.MultitailArrowRenderer(arrow);
      arrowRenderer.show();
    }
  }, {
    ...
    value: function addRxnPlus(rxnPlus) {
      var rxnPlusRenderer = new RxnPlusRenderer.RxnPlusRenderer(rxnPlus);
    ...
    value: function addStereoFlag(stereoFlag) {
      var stereoFlagRenderer = new StereoFlagRenderer.StereoFlagRenderer(stereoFlag);
    ...
    value: function renderAromaticCircles() {
      ...
      var editor = editorSingleton.provideEditorInstance();
      var viewModel = editor.viewModel;
      var canvas = (_ZoomTool$instance = Zoom.ZoomTool.instance) === null || _ZoomTool$instance === void 0 ? void 0 : _ZoomTool$instance.canvas;
    ...
    value: function calculateDashedPolygonPath(loop) {
      ...
      loop.halfEdges.forEach(function (halfEdge, k) {
        ...
        var angle = Math.atan2(vec2.Vec2.cross(halfEdge.direction, nextHalfEdge.direction), vec2.Vec2.dot(halfEdge.direction, nextHalfEdge.direction));
        var halfAngle = (Math.PI - angle) / 2;
        var dir = nextHalfEdge.direction.rotate(halfAngle);
        var pi = scale.Scale.modelToCanvas(nextHalfEdge.firstAtom.position, editorSettings$1);
    ...
    value: function calculateLoopCenterAndRadius(loop) {
      var editorSettings$1 = editorSettings.provideEditorSettings();
      var center = new vec2.Vec2(0, 0);
      loop.halfEdges.forEach(function (halfEdge) {
        var atomPos = scale.Scale.modelToCanvas(halfEdge.firstAtom.position, editorSettings$1);
        center = center.add(atomPos);
      });
      center = center.scaled(1.0 / loop.halfEdges.length);
      var radius = -1;
      loop.halfEdges.forEach(function (halfEdge) {
        var apos = scale.Scale.modelToCanvas(halfEdge.firstAtom.position, editorSettings$1);
        var bpos = scale.Scale.modelToCanvas(halfEdge.secondAtom.position, editorSettings$1);
        var n = vec2.Vec2.diff(bpos, apos).rotateSC(1, 0).normalized();
        var dist = vec2.Vec2.dot(vec2.Vec2.diff(apos, center), n);
...
exports.RenderersManager = RenderersManager;
//# sourceMappingURL=RenderersManager.js.map

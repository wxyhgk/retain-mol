...
var FailedArrowRenderer = function () {
  function FailedArrowRenderer() {
    _classCallCheck__default["default"](this, FailedArrowRenderer);
  }
  _createClass__default["default"](FailedArrowRenderer, null, [{
    key: "preparePaths",
    value: function preparePaths(start, arrowLength, arrowAngle) {
      ...
      var failSignWidth = draw.ARROW_FAIL_SIGN_WIDTH * macroModeScale;
      var arrowCenter = endX - (endX - start.x) / 2;
      var pathParts = [];
      pathParts.push("M".concat(toFixed.toFixed(start.x), ",").concat(toFixed.toFixed(start.y)) + "L".concat(toFixed.toFixed(endX), ",").concat(toFixed.toFixed(start.y)) + "L".concat(toFixed.toFixed(endX - arrowHeadLength), ",").concat(toFixed.toFixed(start.y + arrowHeadWidth)) + "L".concat(toFixed.toFixed(endX - arrowHeadLength + arrowHeadAttr), ",").concat(toFixed.toFixed(start.y)) + "L".concat(toFixed.toFixed(endX - arrowHeadLength), ",").concat(toFixed.toFixed(start.y - arrowHeadWidth)) + "L".concat(toFixed.toFixed(endX), ",").concat(toFixed.toFixed(start.y), "Z"));
      pathParts.push("M".concat(toFixed.toFixed(arrowCenter + failSignWidth), ",").concat(toFixed.toFixed(start.y + failSignWidth)) + "L".concat(toFixed.toFixed(arrowCenter - failSignWidth), ",").concat(toFixed.toFixed(start.y - failSignWidth)));
      pathParts.push("M".concat(toFixed.toFixed(arrowCenter + failSignWidth), ",").concat(toFixed.toFixed(start.y - failSignWidth)) + "L".concat(toFixed.toFixed(arrowCenter - failSignWidth), ",").concat(toFixed.toFixed(start.y + failSignWidth)));
...
exports.FailedArrowRenderer = FailedArrowRenderer;
//# sourceMappingURL=FailedArrowRenderer.js.map

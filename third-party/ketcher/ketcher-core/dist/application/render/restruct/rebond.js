...
var ReBond = function (_ReObject) {
  ...
  function ReBond(_bond) {
    ...
    _defineProperty__default["default"](_assertThisInitialized__default["default"](_this), "b", void 0);
    _defineProperty__default["default"](_assertThisInitialized__default["default"](_this), "doubleBondShift", void 0);
    _defineProperty__default["default"](_assertThisInitialized__default["default"](_this), "path", void 0);
    ...
    _defineProperty__default["default"](_assertThisInitialized__default["default"](_this), "boldStereo", void 0);
    _defineProperty__default["default"](_assertThisInitialized__default["default"](_this), "rbb", void 0);
    _defineProperty__default["default"](_assertThisInitialized__default["default"](_this), "cip", void 0);
    _defineProperty__default["default"](_assertThisInitialized__default["default"](_this), "isPlateShouldBeHidden", function (restruct, options) {
      ReBond.bondRecalc(_assertThisInitialized__default["default"](_this), restruct, options);
      var bond$1 = _this.b;
      var sgroups = restruct.render.ctab.sgroups;
      var functionalGroups = restruct.render.ctab.molecule.functionalGroups;
      if (bond$1.type === bond.Bond.PATTERN.TYPE.HYDROGEN) {
        var beginSgroup = restruct.molecule.getGroupFromAtomId(bond$1.begin);
        var endSgroup = restruct.molecule.getGroupFromAtomId(bond$1.end);
        if (beginSgroup instanceof monomerMicromolecule.MonomerMicromolecule && beginSgroup.monomer.monomerItem.expanded) {
          return true;
        }
        if (endSgroup instanceof monomerMicromolecule.MonomerMicromolecule && endSgroup.monomer.monomerItem.expanded) {
          return true;
        }
      }
      return functionalGroup.FunctionalGroup.isBondInContractedFunctionalGroup(bond$1, sgroups, functionalGroups) || bond.Bond.isBondToHiddenLeavingGroup(restruct.molecule, bond$1);
    });
    _this.b = _bond;
  ...
  _createClass__default["default"](ReBond, [{
    ...
    value: function getSelectionPoints(render) {
      ...
      var halfBondStart = bond$1.hb1 !== undefined ? (_restruct$molecule$ha = restruct.molecule.halfBonds.get(bond$1.hb1)) === null || _restruct$molecule$ha === void 0 ? void 0 : _restruct$molecule$ha.p : undefined;
      var halfBondEnd = bond$1.hb2 !== undefined ? (_restruct$molecule$ha2 = restruct.molecule.halfBonds.get(bond$1.hb2)) === null || _restruct$molecule$ha2 === void 0 ? void 0 : _restruct$molecule$ha2.p : undefined;
      if (!halfBondStart || !halfBondEnd) return [];
      var isStereoBond = bond$1.stereo !== bond.Bond.PATTERN.STEREO.NONE && bond$1.stereo !== bond.Bond.PATTERN.STEREO.CIS_TRANS;
      var highlightPadding = isHighlight ? -1 : 0;
      var stereoPadding = isStereoBond ? 0 : -2;
      var addPadding = highlightPadding + stereoPadding;
      var contourStart = vec2.Vec2.getLinePoint(halfBondEnd, halfBondStart, addPadding);
      var contourEnd = vec2.Vec2.getLinePoint(halfBondStart, halfBondEnd, addPadding);
    ...
    value: function show(restruct, bid, options) {
      ...
      if (functionalGroup.FunctionalGroup.isBondInContractedFunctionalGroup(bond$1, sgroups, functionalGroups)) {
        return;
      }
      if (bond$1.type === bond.Bond.PATTERN.TYPE.HYDROGEN && bond.Bond.isBondToExpandedMonomer(struct, bond$1)) {
        return;
      }
      var paper = render.paper;
      ...
      this.rbb = util["default"].relBox(this.path.getBBox());
      restruct.addReObjectPath(generalEnumTypes.LayerMap.bondSkeleton, this.visel, this.path, null, true);
      var reactingCenter = {
        path: null
      };
      reactingCenter.path = getReactingCenterPath(render, this, hb1, hb2);
      if (reactingCenter.path) {
        reactingCenter.rbb = util["default"].relBox(reactingCenter.path.getBBox());
        restruct.addReObjectPath(generalEnumTypes.LayerMap.data, this.visel, reactingCenter.path, null, true);
      }
      var topology = {
        path: null
      };
      ...
      if (topology.path) {
        topology.rbb = util["default"].relBox(topology.path.getBBox());
        restruct.addReObjectPath(generalEnumTypes.LayerMap.data, this.visel, topology.path, null, true);
      }
      ...
      if (options.showBondIds) {
        ...
        restruct.addReObjectPath(generalEnumTypes.LayerMap.indices, this.visel, ipath);
      }
      ...
      if (bond$1.cip) {
        this.cip = util["default"].drawCIPLabel({
          atomOrBond: bond$1,
          position: bond$1.center,
          restruct: render.ctab,
          visel: this.visel
        });
      }
      this.addTestIds(restruct.molecule, bond$1);
    }
  }, {
    key: "addTestIds",
    value: function addTestIds(struct, bond) {
      ...
      if (_.isNumber(beginSGroupId)) {
        bondPathElement.setAttribute('data-fromsgroupid', beginSGroupId);
        if (struct.sgroups.get(beginSGroupId) instanceof monomerMicromolecule.MonomerMicromolecule) {
          bondPathElement.setAttribute('data-frommonomerid', beginSGroupId);
        }
      }
      if (_.isNumber(endSGroupId)) {
        bondPathElement.setAttribute('data-tosgroupid', endSGroupId);
        if (struct.sgroups.get(endSGroupId) instanceof monomerMicromolecule.MonomerMicromolecule) {
    ...
    value: function drawFragmentSelectionPreview(render, atomIdToDrawArrows, options) {
      ...
      var atom1Position = coordinates.Coordinates.modelToCanvas(atom1.pp);
      var atom2Position = coordinates.Coordinates.modelToCanvas(atom2.pp);
      var bondDirection = halfBond1.p.sub(halfBond2.p).normalized();
      var bondLength = vec2.Vec2.dist(atom2Position, atom1Position);
      ...
      var offsets = [0, -5, -10];
      var contourSize = new vec2.Vec2(bondLength, 20);
      var backgroundStart = new vec2.Vec2(atomIdToDrawArrows === this.b.begin ? atom1Position.x : atom1Position.x + bondLength / 2, atom1Position.y - contourSize.y / 2);
      var newVisel = new visel["default"]('fragment-selection-bond-preview');
      ...
      render.ctab.addReObjectPath(generalEnumTypes.LayerMap.additionalInfo, newVisel, backgroundRect);
      ...
      render.ctab.addReObjectPath(generalEnumTypes.LayerMap.additionalInfo, newVisel, contour);
      render.ctab.movePathOnTopOfLayer(this.visel.paths[0], generalEnumTypes.LayerMap.bondSkeleton);
      contour.rotate(this.b.angle, atom1Position.x, atom1Position.y);
      var arrowsSet = render.paper.set();
      offsets.forEach(function (delta) {
        var start = new vec2.Vec2(arrowStart.x - arrowsOffsetFromContour * arrowsDirection.x + delta * arrowsDirection.x, arrowStart.y - arrowsOffsetFromContour * arrowsDirection.y + delta * arrowsDirection.y);
      ...
      render.ctab.addReObjectPath(generalEnumTypes.LayerMap.additionalInfo, newVisel, arrowsSet);
      return newVisel;
    }
  }], [{
    key: "isSelectable",
    ...
    value: function bondRecalc(bond, restruct, options) {
      ...
      bond.b.center = vec2.Vec2.lc2(p1, 0.5, p2, 0.5);
      bond.b.len = vec2.Vec2.dist(scale.Scale.modelToCanvas(p1, render.options), scale.Scale.modelToCanvas(p2, render.options));
...
function findIncomingStereoUpBond(atom, bid0, includeBoldStereoBond, restruct) {
  return atom.neighbors.findIndex(function (hbid) {
    ...
    var singleUp = neibond.b.type === bond.Bond.PATTERN.TYPE.SINGLE && neibond.b.stereo === bond.Bond.PATTERN.STEREO.UP;
    if (singleUp) {
      if (bond.Bond.isBondToHiddenLeavingGroup(restruct.molecule, neibond.b)) {
        return false;
      }
      return neibond.b.end === hb.begin || neibond.boldStereo && includeBoldStereoBond;
    }
    if (neibond.b.type === bond.Bond.PATTERN.TYPE.DOUBLE && neibond.b.stereo === bond.Bond.PATTERN.STEREO.NONE && includeBoldStereoBond && neibond.boldStereo) {
      return !bond.Bond.isBondToHiddenLeavingGroup(restruct.molecule, neibond.b);
    }
    return false;
  });
}
...
function getBondPath(restruct, bond$1, hb1, hb2, isSnapping) {
  ...
  var xShiftMinus1 = -1;
  var xShiftPlus1 = 1;
  switch (bond$1.b.type) {
    case bond.Bond.PATTERN.TYPE.SINGLE:
      switch (bond$1.b.stereo) {
        case bond.Bond.PATTERN.STEREO.UP:
          findIncomingUpBonds(hb1.bid, bond$1, restruct);
          if (bond$1.boldStereo && bond$1.neihbid1 >= 0 && bond$1.neihbid2 >= 0) {
            path = getBondSingleStereoBoldPath(render, hb1, hb2, bond$1, struct, isSnapping);
          } else path = getBondSingleUpPath(render, hb1, hb2, bond$1, struct, isSnapping);
          break;
        case bond.Bond.PATTERN.STEREO.DOWN:
          path = getBondSingleDownPath(render, hb1, hb2, bond$1, struct, isSnapping);
          break;
        case bond.Bond.PATTERN.STEREO.EITHER:
          path = getBondSingleEitherPath(render, hb1, hb2, bond$1, struct, isSnapping);
          break;
        default:
          path = draw["default"].bondSingle(render.paper, hb1, hb2, render.options, isSnapping, getStereoBondColor(render.options, bond$1, struct));
          break;
      }
      break;
    case bond.Bond.PATTERN.TYPE.DOUBLE:
      findIncomingUpBonds(hb1.bid, bond$1, restruct);
      if (bond$1.b.stereo === bond.Bond.PATTERN.STEREO.NONE && bond$1.boldStereo && bond$1.neihbid1 >= 0 && bond$1.neihbid2 >= 0) {
        path = getBondDoubleStereoBoldPath(render, hb1, hb2, bond$1, struct, shiftA, shiftB, isSnapping);
      } else path = getBondDoublePath(render, hb1, hb2, bond$1, shiftA, shiftB, isSnapping);
      break;
    case bond.Bond.PATTERN.TYPE.TRIPLE:
      path = draw["default"].bondTriple(render.paper, hb1, hb2, render.options, isSnapping);
      break;
    case bond.Bond.PATTERN.TYPE.AROMATIC:
      {
        ...
        path = inAromaticLoop ? draw["default"].bondSingle(render.paper, hb1, hb2, render.options, isSnapping) : getBondAromaticPath(render, hb1, hb2, bond$1, shiftA, shiftB, isSnapping);
        break;
      }
    case bond.Bond.PATTERN.TYPE.SINGLE_OR_DOUBLE:
      newHalfBonds = util["default"].updateHalfBondCoordinates(hb1, hb2, xShiftPlus1);
      path = getSingleOrDoublePath(render, newHalfBonds[0], newHalfBonds[1], isSnapping);
      break;
    case bond.Bond.PATTERN.TYPE.SINGLE_OR_AROMATIC:
      path = getBondAromaticPath(render, hb1, hb2, bond$1, shiftA, shiftB, isSnapping);
      break;
    case bond.Bond.PATTERN.TYPE.DOUBLE_OR_AROMATIC:
      newHalfBonds = util["default"].updateHalfBondCoordinates(hb1, hb2, xShiftMinus1);
      path = getBondAromaticPath(render, newHalfBonds[0], newHalfBonds[1], bond$1, shiftA, shiftB, isSnapping);
      break;
    case bond.Bond.PATTERN.TYPE.ANY:
      newHalfBonds = util["default"].updateHalfBondCoordinates(hb1, hb2, xShiftMinus1);
      path = draw["default"].bondAny(render.paper, newHalfBonds[0], newHalfBonds[1], render.options, isSnapping);
      break;
    case bond.Bond.PATTERN.TYPE.HYDROGEN:
      newHalfBonds = util["default"].updateHalfBondCoordinates(hb1, hb2, xShiftPlus1);
      path = draw["default"].bondHydrogen(render.paper, newHalfBonds[0], newHalfBonds[1], render.options, isSnapping);
      break;
    case bond.Bond.PATTERN.TYPE.DATIVE:
      path = draw["default"].bondDative(render.paper, hb1, hb2, render.options, isSnapping);
      break;
    default:
      throw new Error('Bond type ' + bond$1.b.type + ' not supported');
  }
  if (path) {
    var previewOpacity = render.options.previewOpacity;
    path.attr({
      opacity: bond$1.b.isPreview ? previewOpacity : 1
    });
  }
  return path;
}
function getBondSingleUpPath(render, hb1, hb2, bond, struct, isSnapping) {
  ...
  return draw["default"].bondSingleUp(render.paper, a, b2, b3, options, isSnapping, getStereoBondColor(options, bond, struct));
}
function getStereoBondColor(options, bond, struct) {
  ...
  if (
  !stereoLabel || options.colorStereogenicCenters === generalEnumTypes.StereoColoringType.Off || options.colorStereogenicCenters === generalEnumTypes.StereoColoringType.LabelsOnly) {
    return defaultColor;
  }
  return (_getColorFromStereoLa = reatom.getColorFromStereoLabel(options, stereoLabel)) !== null && _getColorFromStereoLa !== void 0 ? _getColorFromStereoLa : defaultColor;
}
function getBondSingleStereoBoldPath(render, hb1, hb2, bond, struct, isSnapping) {
  ...
  var a3 = coords2[0];
  var a4 = coords2[1];
  return draw["default"].bondSingleStereoBold(render.paper, a1, a2, a3, a4, options, isSnapping, getStereoBondColor(options, bond, struct));
}
function getBondDoubleStereoBoldPath(render, hb1, hb2, bond, struct, shiftA, shiftB, isSnapping) {
  ...
  return draw["default"].bondDoubleStereoBold(render.paper, sgBondPath, b1, b2, render.options, isSnapping, getStereoBondColor(render.options, bond, struct));
}
...
function stereoUpBondGetCoordinates(hb, neihbid, bondSpace, struct) {
  ...
  var cos = vec2.Vec2.dot(hb.dir, neihb.dir);
  var sin = vec2.Vec2.cross(hb.dir, neihb.dir);
...
function getBondSingleDownPath(render, hb1, hb2, bond, struct, isSnapping) {
  ...
  return draw["default"].bondSingleDown(render.paper, hb1, d, finalLines, step, options, isSnapping, getStereoBondColor(options, bond, struct));
}
function getBondSingleEitherPath(render, hb1, hb2, bond, struct, isSnapping) {
  ...
  return draw["default"].bondSingleEither(render.paper, hb1, d, nlines, step, options, isSnapping, getStereoBondColor(options, bond, struct));
}
function getBondDoublePath(render, hb1, hb2, bond$1, shiftA, shiftB, isSnapping) {
  var cisTrans = bond$1.b.stereo === bond.Bond.PATTERN.STEREO.CIS_TRANS;
  var a = hb1.p;
  var b = hb2.p;
  var n = hb1.norm;
  var shift = cisTrans ? 0 : bond$1.doubleBondShift;
  var options = render.options;
  ...
  return draw["default"].bondDouble(render.paper, a1, a2, b1, b2, cisTrans, options, isSnapping);
}
function getSingleOrDoublePath(render, hb1, hb2, isSnapping) {
  var a = hb1.p;
  var b = hb2.p;
  var options = render.options;
  var nSect = vec2.Vec2.dist(a, b) / Number((options.bondSpace + options.lineWidth).toFixed());
  if (!(nSect & 1)) nSect += 1;
  return draw["default"].bondSingleOrDouble(render.paper, hb1, hb2, nSect, options, isSnapping);
}
function getBondAromaticPath(render, hb1, hb2, bond$1, shiftA, shiftB, isSnapping) {
  var dashdotPattern = [0.125, 0.125, 0.005, 0.125];
  var mask = 0;
  var dash = null;
  var options = render.options;
  var bondShift = bond$1.doubleBondShift;
  if (bond$1.b.type === bond.Bond.PATTERN.TYPE.SINGLE_OR_AROMATIC) {
    mask = bondShift > 0 ? 1 : 2;
    dash = dashdotPattern.map(function (v) {
      return v * options.microModeScale;
    });
  }
  if (bond$1.b.type === bond.Bond.PATTERN.TYPE.DOUBLE_OR_AROMATIC) {
    mask = 3;
    dash = dashdotPattern.map(function (v) {
      return v * options.microModeScale;
    });
  ...
  return draw["default"].bondAromatic(render.paper, paths, bondShift, options, isSnapping);
}
function getAromaticBondPaths(hb1, hb2, shift, shiftA, shiftB, bondSpace, mask, dash) {
  ...
  return draw["default"].aromaticBondPaths(a2, a3, b2, b3, mask, dash);
}
function getReactingCenterPath(render, bond$1, hb1, hb2) {
  var a = hb1.p;
  var b = hb2.p;
  ...
  var acrossSz = 3.0 * bs;
  var tiltTan = 0.2;
  switch (bond$1.b.reactingCenterStatus) {
  ...
  return draw["default"].reactingCenter(render.paper, p, render.options);
}
function getBondMark(render, bond$1, hb1, hb2) {
  var options = render.options;
  var mark = null;
  var tooltip = null;
  if (bond$1.b.customQuery) {
    mark = bond$1.b.customQuery;
    if (bond$1.b.customQuery.length > 8) {
      tooltip = bond$1.b.customQuery;
      mark = "".concat(bond$1.b.customQuery.substring(0, 8), "...");
    }
  } else if (bond$1.b.topology === bond.Bond.PATTERN.TOPOLOGY.RING) {
    mark = 'rng';
  } else if (bond$1.b.topology === bond.Bond.PATTERN.TOPOLOGY.CHAIN) {
    mark = 'chn';
  } else {
    return null;
  }
  ...
  if (bond$1.doubleBondShift > 0) n = n.scaled(-bond$1.doubleBondShift);else if (bond$1.doubleBondShift === 0) fixed += options.bondSpace / 2;
  var s = new vec2.Vec2(2, 1).scaled(options.bondSpace);
  if (bond$1.b.type === bond.Bond.PATTERN.TYPE.TRIPLE) fixed += options.bondSpace;
  var p = c.add(new vec2.Vec2(n.x * (s.x + fixed), n.y * (s.y + fixed)));
  var path = draw["default"].bondMark(render.paper, p, mark, options);
  if (tooltip) {
    path.node.childNodes[0].setAttribute('data-tooltip', util["default"].escapeHtml(tooltip));
  }
  return path;
}
function getIdsPath(bid, paper, hb1, hb2, bondIdxOff, param1, param2, norm) {
  var pb = vec2.Vec2.lc(hb1.p, param1, hb2.p, param2, norm, bondIdxOff);
  var ipath = paper.text(pb.x, pb.y, bid.toString());
  var irbb = util["default"].relBox(ipath.getBBox());
  draw["default"].recenterText(ipath, irbb);
  return ipath;
}
...
exports.getBondLineShift = getBondLineShift;

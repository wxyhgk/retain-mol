...
import { Atom, StereoLabel } from '../../../domain/entities/atom.modern.js';
...
import { ElementColor } from '../../../domain/constants/elementColor.modern.js';
...
import { LayerMap, StereoColoringType, StereoLabelStyleType } from './generalEnumTypes.modern.js';
...
import { getAttachmentPointLabel } from '../../../domain/helpers/attachmentPointCalculations.modern.js';
...
import { UsageInMacromolecule } from '../render.constants.modern.js';
import { MonomerMicromolecule } from '../../../domain/entities/monomerMicromolecule.modern.js';
import { attachmentPointNames } from '../../../domain/types/monomers.modern.js';
import '../../../domain/types/entities.modern.js';
import { VALENCE_MAP } from './constants.modern.js';
import { SUPERATOM_CLASS_TEXT } from './resgroup.modern.js';
import { getAttachmentPointTooltip } from '../../../domain/helpers/attachmentPointTooltips.modern.js';
import { ShowHydrogenLabels } from './showHydrogenLabels.modern.js';
...
var ReAtom = function (_ReObject) {
  ...
  function ReAtom(_atom) {
    ...
    _defineProperty(_assertThisInitialized(_this), "a", void 0);
    _defineProperty(_assertThisInitialized(_this), "showLabel", void 0);
    _defineProperty(_assertThisInitialized(_this), "showInfoLabel", void 0);
    _defineProperty(_assertThisInitialized(_this), "hydrogenOnTheLeft", void 0);
    ...
    _defineProperty(_assertThisInitialized(_this), "infoLabel", void 0);
    ...
    _defineProperty(_assertThisInitialized(_this), "isPlateShouldBeHidden", function (atom, render) {
      var sgroups = render.ctab.sgroups;
      var functionalGroups = render.ctab.molecule.functionalGroups;
      var struct = render.ctab.molecule;
      var atomId = struct.atoms.keyOf(atom);
      return FunctionalGroup.isAtomInContractedFunctionalGroup(atom, sgroups, functionalGroups) || Atom.isHiddenLeavingGroupAtom(struct, atomId);
    });
  ...
  _createClass(ReAtom, [{
    ...
    value: function attachHighlightTriggerForAttachmentPointAtom(hoverElement, render) {
      ...
      var attachmentPointEntry = Array.from(assignedAttachmentPoints.entries()).find(function (_ref) {
        var _ref2 = _slicedToArray(_ref, 2),
          atomsPair = _ref2[1];
        var _atomsPair = _slicedToArray(atomsPair, 2),
    ...
    value: function drawHoverForPotentialAttachmentPointAtomsInMonomerCreationWizard(render) {
      ...
      var isAtomInAssignedAttachmentPoint = Array.from(assignedAttachmentPoints.values()).some(function (atomsPair) {
        var _atomsPair2 = _slicedToArray(atomsPair, 2),
    ...
    value: function setHover(hover, render) {
      ...
      _get(_getPrototypeOf(ReAtom.prototype), "setHover", this).call(this, hover, render, drawOutline);
    ...
    value: function createInvisibleAtomTarget(restruct, render, position) {
      ...
      restruct.addReObjectPath(LayerMap.data, this.visel, invisibleAtomTarget, position);
    ...
    value: function getShiftedSegmentPosition(renderOptions, direction, _atomPosition) {
      ...
      var atomPosition = Scale.modelToCanvas(_atomPosition !== null && _atomPosition !== void 0 ? _atomPosition : this.a.pp, renderOptions);
      ...
      try {
        for (_iterator.s(); !(_step = _iterator.n()).done;) {
          ...
          var shiftRayBox = util.shiftRayBox(atomPosition, direction, box);
    ...
    value: function show(restruct, aid, options) {
      ...
      if (FunctionalGroup.isAtomInContractedFunctionalGroup(atom, sgroups, functionalGroups)) {
        if (sgroup) {
          ...
          if (isPositionAtom) {
            ...
            var position = Scale.modelToCanvas(contractedPosition, render.options);
            ...
            var sGroupName = (_ref3 = (_sgroup$data$name = (_sgroup$data2 = sgroup.data) === null || _sgroup$data2 === void 0 ? void 0 : _sgroup$data2.name) !== null && _sgroup$data$name !== void 0 ? _sgroup$data$name : superatomClass ? SUPERATOM_CLASS_TEXT[superatomClass] : '') !== null && _ref3 !== void 0 ? _ref3 : '';
            ...
            restruct.addReObjectPath(LayerMap.data, this.visel, path, position, true);
          }
          return;
        }
      }
      ...
      if (this.showLabel) {
        ...
        if (label.background) {
          restruct.addReObjectPath(LayerMap.data, this.visel, label.background, ps, true);
        }
      ...
      if (options.showAtomIds) {
        ...
        var rbb = util.relBox(_path.getBBox());
        draw.recenterText(_path, rbb);
        index = {
          text: _text,
          path: _path,
          rbb: rbb
      ...
      if (this.showLabel) {
        ...
        var isPreviewMode = options.usageInMacromolecule === UsageInMacromolecule.MonomerPreview || options.usageInMacromolecule === UsageInMacromolecule.BondPreview || options.usageInMacromolecule === UsageInMacromolecule.MonomerConnectionsModal || options.usageInMacromolecule === undefined && !sgroup;
      ...
      if (render.monomerCreationState) {
        ...
        if (_aid !== null) {
          ...
          if (style) {
            ...
            _restruct.addReObjectPath(LayerMap.atom, this.visel, _path2);
          }
          if (problematicAtoms !== null && problematicAtoms !== void 0 && problematicAtoms.has(_aid)) {
            ...
            _restruct.addReObjectPath(LayerMap.atom, this.visel, _path3);
          }
          ...
          if (attachmentPointName) {
            ...
            assert(atomsPair);
            var _atomsPair3 = _slicedToArray(atomsPair, 2),
            ...
            var apTooltip = getAttachmentPointTooltip(selectedClass, attachmentPointName);
            if (apTooltip) {
            ...
            _restruct.addReObjectPath(LayerMap.data, this.visel, labelGroup, ps, false);
          }
          if (connectionAttachmentPoints) {
            ...
            if (isConnectionAtom) {
              ...
              _restruct.addReObjectPath(LayerMap.atom, this.visel, ringPath);
              ...
              _restruct.addReObjectPath(LayerMap.data, this.visel, hitArea, ps, false);
            }
          }
        }
      }
      ...
      if (text.length > 0) {
        ...
        var aamPath = render.paper.text(ps.x, ps.y, text).attr({
          font: options.font,
          'font-size': options.fontszsubInPx,
          fill: options.atomColoring && elem ? ElementColor[this.a.label] : DEFAULT_ATOM_COLOR
        });
        if (stereoLabel) {
      ...
      if (isHighlighted && !highlightOutline) {
        ...
        restruct.addReObjectPath(LayerMap.hovering, this.visel, _path4);
      }
      if (atom.cip) {
        var paper = render.paper;
        var _options = render.options;
        var _ps = Scale.modelToCanvas(this.a.pp, _options);
        ...
        var cipGroupRelBox = util.relBox(cipGroup.getBBox());
        var baseDistance = 3;
        ...
        try {
          for (_iterator3.s(); !(_step3 = _iterator3.n()).done;) {
            ...
            baseDistance = Math.max(baseDistance, util.shiftRayBox(_ps, _direction, _ext.translate(_ps)));
          }
        } catch (err) {
          _iterator3.e(err);
        } finally {
        ...
        var shiftDistance = baseDistance + util.shiftRayBox(_ps, _direction.negated(), Box2Abs.fromRelBox(cipTextBBox));
        ...
        render.ctab.addReObjectPath(LayerMap.additionalInfo, this.visel, cipGroup, _ps, false);
        this.cip = {
          path: cipGroup,
          text: cipText,
          rectangle: rect
      ...
      if (this.showLabel && this.showInfoLabel) {
        ...
        restruct.addReObjectPath(LayerMap.data, this.visel, [_rect, _path5], ps, true);
      }
...
function buildLabel(atom, paper, ps, options, atomId, sgroup) {
  ...
  if (text === atom.a.label) {
    var element = Elements.get(text);
    if (atomColoring && element) {
      var _ElementColor$text;
      atom.color = (_ElementColor$text = ElementColor[text]) !== null && _ElementColor$text !== void 0 ? _ElementColor$text : DEFAULT_ATOM_COLOR;
    }
  }
  var shouldStyleLabel = usageInMacromolecule !== undefined;
  var isMonomerAttachmentPoint = attachmentPointNames.includes(text);
  ...
  var _util$useLabelStyles = util.useLabelStyles(isMonomerAttachmentPointSelected, isMonomerAttachmentPointUsed, usageInMacromolecule !== null && usageInMacromolecule !== void 0 ? usageInMacromolecule : UsageInMacromolecule.MonomerConnectionsModal),
...
function getLabelText(atom, atomId, sgroup, options) {
  if (sgroup !== null && sgroup !== void 0 && sgroup.isSuperatomWithoutLabel) {
    ...
    if (attachmentPoint !== null && attachmentPoint !== void 0 && attachmentPoint.attachmentPointNumber) {
      var result = getAttachmentPointLabel(attachmentPoint.attachmentPointNumber);
      return result;
    }
  }
  ...
  if (atom.label && atom.rglabel !== null && sgroup instanceof MonomerMicromolecule) {
  ...
  if (atom.label && atom.rglabel !== null) {
    var text = '';
    var rglabelNum = atom.rglabel;
    for (var rgi = 0; rgi < 32; rgi++) {
      if (rglabelNum & 1 << rgi) {
        text += 'R' + (rgi + 1).toString();
      }
    }
    if (sgroup instanceof MonomerMicromolecule && Atom.isSuperatomLeavingGroupAtom(sgroup, atomId)) {
...
function showRadical(atom, render) {
  ...
  switch (atom.a.radical) {
    case 1:
      ...
      path.push(draw.radicalBullet(paper, ps.add(new Vec2(-hshift, 0)), options), draw.radicalBullet(paper, ps.add(new Vec2(hshift, 0)), options));
    ...
    case 3:
      path = paper.set();
      hshift = 1.6 * options.lineWidth;
      path.push(draw.radicalCap(paper, ps.add(new Vec2(-hshift, 0)), options), draw.radicalCap(paper, ps.add(new Vec2(hshift, 0)), options));
...
function showExplicitValence(atom, render, rightMargin) {
  ...
  var options = render.options;
  var delta = 0.5 * options.lineWidth;
  var baseText = VALENCE_MAP[atom.a.explicitValence];
  if (!baseText) {
...
function showHydrogen(atom, render, implh, data) {
  ...
  var hydrogenRbb = util.relBox(hydrogenPath.getBBox());
  ...
  if (implh > 1) {
    ...
    var hydroIndexRbb = util.relBox(hydroIndexPath.getBBox());
...
function showWarning(atom, render, leftMargin, rightMargin) {
  ...
  var path = render.paper.path('M{0},{1}L{2},{3}', toFixed(ps.x + leftMargin), toFixed(y), toFixed(ps.x + rightMargin), toFixed(y)).attr(render.options.lineattr).attr({
    stroke: '#F00'
  });
  var rbb = util.relBox(path.getBBox());
  return {
...
export { ShowHydrogenLabelNames, checkIsSmartPropertiesExist, ReAtom as default, getAtomCustomQuery, getAtomType, getColorFromStereoLabel };
//# sourceMappingURL=reatom.modern.js.map

...
function collectMonomerBonds(monomer) {
  ...
  for (var apName in monomer.attachmentPointsToBonds) {
    ...
    } else if (bond instanceof MonomerToAtomBond.MonomerToAtomBond) {
      otherEntity = null;
    }
    if (otherEntity) {
      records.push({
...
function getPresetPhosphateFromSugar(sugar, position) {
  ...
  if (bond instanceof PolymerBond.PolymerBond) {
    ...
    if (other && monomers.isPhosphateOrAmbiguousPhosphate(other) && other.getAttachmentPointByBond(bond) === phosphateAP) {
      return other;
    }
  }
  return null;
...
function getPresetSugarForMonomer(monomer, libraryPreset) {
  if (monomers.isSugarOrAmbiguousSugar(monomer)) {
    return monomer;
  }
  if (monomers.isRnaBaseOrAmbiguousRnaBase(monomer)) {
    var _getSugarFromRnaBase;
    return (_getSugarFromRnaBase = monomers.getSugarFromRnaBase(monomer)) !== null && _getSugarFromRnaBase !== void 0 ? _getSugarFromRnaBase : null;
  }
  if (monomers.isPhosphateOrAmbiguousPhosphate(monomer)) {
    var _libraryPreset$phosph;
    var preferredAP = ((_libraryPreset$phosph = libraryPreset.phosphatePosition) !== null && _libraryPreset$phosph !== void 0 ? _libraryPreset$phosph : 'right') === 'left' ? monomers$1.AttachmentPointName.R2 : monomers$1.AttachmentPointName.R1;
    var apOrder = preferredAP === monomers$1.AttachmentPointName.R1 ? [monomers$1.AttachmentPointName.R1, monomers$1.AttachmentPointName.R2] : [monomers$1.AttachmentPointName.R2, monomers$1.AttachmentPointName.R1];
    for (var _i = 0, _apOrder = apOrder; _i < _apOrder.length; _i++) {
      var apName = _apOrder[_i];
      var bond = monomer.attachmentPointsToBonds[apName];
      if (bond instanceof PolymerBond.PolymerBond) {
        var other = bond.getAnotherMonomer(monomer);
        if (other && monomers.isSugarOrAmbiguousSugar(other)) {
          return other;
        }
      }
    }
...
function getMatchingPresetComponents(sugar, libraryPreset) {
  if (!monomers.isSugarOrAmbiguousSugar(sugar)) return null;
  var components = [sugar];
  if (libraryPreset.base) {
    var base = monomers.getRnaBaseFromSugar(sugar);
    if (!base) return null;
...
function getPresetComponentRole(monomer) {
  if (monomers.isSugarOrAmbiguousSugar(monomer)) return 'sugar';
  if (monomers.isRnaBaseOrAmbiguousRnaBase(monomer)) return 'base';
  if (monomers.isPhosphateOrAmbiguousPhosphate(monomer)) return 'phosphate';
  return null;
}
  ...
  try {
    for (_iterator4.s(); !(_step4 = _iterator4.n()).done;) {
      ...
      if (!role || !rolePresent[role]) {
        lost.push.apply(lost, _toConsumableArray__default["default"](externalBonds));
        continue;
      }
...
exports.collectMonomerBonds = collectMonomerBonds;
exports.computeLostBondsForMonomerReplacement = computeLostBondsForMonomerReplacement;
exports.computeLostBondsForPresetReplacement = computeLostBondsForPresetReplacement;
exports.computeReestablishableBonds = computeReestablishableBonds;
exports.getMatchingPresetComponents = getMatchingPresetComponents;
exports.getPresetComponentRole = getPresetComponentRole;
exports.getPresetPhosphateFromSugar = getPresetPhosphateFromSugar;
exports.getPresetSugarForMonomer = getPresetSugarForMonomer;
//# sourceMappingURL=replacementHelpers.js.map

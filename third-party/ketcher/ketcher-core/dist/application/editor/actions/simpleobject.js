...
function fromSimpleObjectAddition(restruct, pos, mode, toCircle) {
  var action$1 = new action.Action();
  action$1.addOp(new simpleObject.SimpleObjectAdd(pos, mode, toCircle));
  return action$1.perform(restruct);
}
function fromSimpleObjectResizing(restruct, id, d, current, anchor, toCircle) {
  var action$1 = new action.Action();
  action$1.addOp(new simpleObject.SimpleObjectResize(id, d, current, anchor, false, toCircle));
  return action$1.perform(restruct);
}

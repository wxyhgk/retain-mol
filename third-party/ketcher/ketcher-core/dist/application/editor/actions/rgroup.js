...
var UpdateIfThen = require('../operations/UpdateIfThen.js');

function fromRGroupAttrs(restruct, id, attrs) {
  var action$1 = new action.Action();
  Object.keys(attrs).forEach(function (key) {
    action$1.addOp(new RGroupAttr.RGroupAttr(id, key, attrs[key]));
  });
  return action$1.perform(restruct);
}
function fromRGroupFragment(restruct, rgidNew, frid) {
  var action$1 = new action.Action();
  action$1.addOp(new RGroupFragment.RGroupFragment(rgidNew, frid));
  return action$1.perform(restruct);
}
function fromUpdateIfThen(restruct, rgidNew, rgidOld) {
  ...
  if (!restruct.molecule.rgroups.get(rgidOld)) {
    action$1.addOp(new UpdateIfThen.UpdateIfThen(rgidNew, rgidOld, skipRgids));
  }
  return action$1.perform(restruct);
}

...
var AlignDescriptors = require('../operations/AlignDescriptors.js');

function fromNewCanvas(restruct, struct) {
  var action$1 = new action.Action();
  action$1.addOp(new CanvasLoad.CanvasLoad(struct));
  return action$1.perform(restruct);
}
function fromDescriptorsAlign(restruct) {
  var action$1 = new action.Action();
  action$1.addOp(new AlignDescriptors.AlignDescriptors());
  return action$1.perform(restruct);
}

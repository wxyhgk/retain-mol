...
var RxnArrowResize = require('../operations/rxn/RxnArrowResize.js');
...
function fromArrowAddition(restruct, pos, mode) {
  ...
  action$1.addOp(new index.RxnArrowAdd(pos, mode));
  return action$1.perform(restruct);
}
function fromArrowResizing(restruct, id, d, current, anchor, isSnappingEnabled) {
  var action$1 = new action.Action();
  action$1.addOp(new RxnArrowResize.RxnArrowResize(id, d, current, anchor, false, isSnappingEnabled));
  return action$1.perform(restruct);
}
...
function fromPlusAddition(restruct, pos) {
  var action$1 = new action.Action();
  action$1.addOp(new index$1.RxnPlusAdd(pos).perform(restruct));
  return action$1;
}

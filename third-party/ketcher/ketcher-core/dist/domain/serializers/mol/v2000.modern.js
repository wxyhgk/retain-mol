 ...
 ***************************************************************************/
import { StereoLabel, Atom, AttachmentPoints } from '../../entities/atom.modern.js';
import { AtomList } from '../../entities/atomList.modern.js';
...
import sGroup from './parseSGroup.modern.js';
...
function handleSubstitutionProperty(propertyData, props) {
  ...
  var arrs = sGroup.readKeyMultiValuePairs(propertyData, false);
...
function handleRGroupProperty(propertyData, props) {
  ...
  var a2rs = sGroup.readKeyMultiValuePairs(propertyData, false);
...
function handleRGroupLogic(propertyData, rLogic) {
  ...
  var rgid = utils.parseDecimalInt(data.slice(0, 3).trim());
  var iii = utils.parseDecimalInt(data.slice(4, 7).trim());
  var hhh = utils.parseDecimalInt(data.slice(8, 11).trim());
...
function createRGroupAttachmentPointsFromAtoms(struct) {
  struct.atoms.forEach(function (atom, atomId) {
    ...
    var attachmentPoints = atom.attachmentPoints;
    if (attachmentPoints === AttachmentPoints.FirstSideOnly) {
      struct.rgroupAttachmentPoints.add(new RGroupAttachmentPoint(atomId, 'primary'));
    } else if (attachmentPoints === AttachmentPoints.SecondSideOnly) {
      struct.rgroupAttachmentPoints.add(new RGroupAttachmentPoint(atomId, 'secondary'));
  ...
  bonds.forEach(function (bond) {
    ...
    if (beginAtom) {
      ...
      if (bond.stereo && isAnd) {
        beginAtom.stereoLabel = "".concat(StereoLabel.And, "1");
      }
    }
    ctab.bonds.add(bond);
  });
...
function labelsListToIds(labels) {
  ...
  try {
    for (_iterator4.s(); !(_step4 = _iterator4.n()).done;) {
      ...
      var element = Elements.get(label.trim());
      if (element) {
...
export { v2000 as default };
//# sourceMappingURL=v2000.modern.js.map

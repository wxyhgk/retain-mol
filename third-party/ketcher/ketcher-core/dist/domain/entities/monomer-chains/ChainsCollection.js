...
var ChainsCollection = function () {
  ...
  _createClass__default["default"](ChainsCollection, [{
    ...
    value: function rearrange() {
      ...
      this.chains = _toConsumableArray__default["default"](reorderedChains.values());
    ...
    value: function findCycledComplimentaryChains(chain, startChain, previousChain) {
      ...
      if (complimentaryChainGoesToStartChain) {
        ...
        return complimentaryChainsWithData.reduce(function (acc, _ref5) {
          ...
          return [].concat(_toConsumableArray__default["default"](acc), _toConsumableArray__default["default"](_this.findCycledComplimentaryChains(complimentaryChain, startChain, chain, visitedChains)));
        }, []);
      }
    }
  }, {
    ...
    value: function getComplimentaryChainIfNucleotide(node, monomerToChain, monomerToNode) {
      ...
      try {
        for (_iterator.s(); !(_step = _iterator.n()).done;) {
          ...
          var isRnaMonomer = monomers.isRnaBaseApplicableForAntisense(monomer);
          var isRnaComplimentaryMonomer = monomers.isRnaBaseApplicableForAntisense(complimentaryMonomer);
    ...
    value: function fromMonomers(monomers) {
      ...
      firstMonomersInRegularChains.forEach(function (monomer) {
        chainsCollection.add(new Chain.Chain(monomer));
      });
      firstMonomersInCycledChains.forEach(function (monomer) {
        chainsCollection.add(new Chain.Chain(monomer, !!types.IsChainCycled.CYCLED));
      });
      ...
      if (firstMonomersInMiddleOfChains.length) {
        firstMonomersInMiddleOfChains.forEach(function (firstMonomerInMiddleOfChain) {
          chainsCollection.add(new Chain.Chain(firstMonomerInMiddleOfChain));
        });
      }
      return chainsCollection;
    }
    ...
    value: function getFirstMonomersInChains(monomers) {
      var MonomerTypes = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : [Peptide.Peptide, Chem.Chem, Phosphate.Phosphate, Sugar.Sugar, RNABase.RNABase, UnresolvedMonomer.UnresolvedMonomer, UnsplitNucleotide.UnsplitNucleotide, AmbiguousMonomer.AmbiguousMonomer];
    ...
    value: function getFirstMonomersInMiddleOfChains(monomers$1) {
      var initialMonomersSet = new Set(monomers$1);
      var handledMonomers = new Set();
      var firstMonomersInMiddleOfChains = [];
      monomers$1.forEach(function (monomer) {
        ...
        while (previousMonomerInChain && !handledMonomers.has(previousMonomerInChain) && !initialMonomersSet.has(previousMonomerInChain)) {
          var previousMonomer = monomers.getPreviousMonomerInChain(previousMonomerInChain);
    ...
    value: function getFirstMonomersInRegularChains(monomersList) {
      var firstMonomersInRegularChains = monomersList.filter(function (monomer) {
        ...
        var isRnaBaseConnectedToSugar = monomers.isRnaBaseOrAmbiguousRnaBase(monomer) && R1ConnectedMonomer instanceof Sugar.Sugar && monomers.getRnaBaseFromSugar(R1ConnectedMonomer) === monomer;
        var isStart = (isFirstMonomerWithR2R1connection || monomers.isMonomerConnectedToR2RnaBase(monomer)) && !isRnaBaseConnectedToSugar;
        if (isStart && !monomers.isMonomerConnectedToR2RnaBase(monomer)) {
          var previousMonomer = monomers.getPreviousMonomerInChain(monomer);
          if (previousMonomer && (monomers.isLinearChem(monomer) || monomers.isLinearChem(previousMonomer))) {
            return false;
          }
        }
        return isStart;
    ...
    value: function getFirstMonomersInCycledChains(monomersList) {
      ...
      monomersList.forEach(function (monomer) {
        ...
        while (nextMonomerInChain && !handledMonomers.has(nextMonomerInChain)) {
          ...
          nextMonomerInChain = monomers.getNextMonomerInChain(nextMonomerInChain);
        }
...
exports.ChainsCollection = ChainsCollection;
//# sourceMappingURL=ChainsCollection.js.map

...
var MultitailArrow = function (_BaseMicromoleculeEnt) {
  _inherits(MultitailArrow, _BaseMicromoleculeEnt);
  ...
  _createClass(MultitailArrow, [{
    ...
    value: function getReferenceLines(referencePositions) {
      ...
      var headSpinePosition = new Vec2(spineX, referencePositions.head.y);
      var tails = new Pool();
    ...
    value: function getTailsDistance(tailsYOffsets) {
      var allTailsOffsets = tailsYOffsets.concat([new FixedPrecisionCoordinates(0), this.height]);
    ...
    value: function addTail(id, coordinate) {
      ...
      var centerFixedPrecision = FixedPrecisionCoordinates.fromFloatingPrecision(center);
      if (typeof id === 'number') {
    ...
    value: function resizeHead(offset) {
      var fixedPrecisionOffset = FixedPrecisionCoordinates.fromFloatingPrecision(offset);
      var headOffsetX = new FixedPrecisionCoordinates(Math.max(this.headOffsetX.add(fixedPrecisionOffset).value, MultitailArrow.MIN_HEAD_LENGTH.value));
    ...
    value: function moveHead(offset) {
      var fixedPrecisionOffset = FixedPrecisionCoordinates.fromFloatingPrecision(offset);
      var headOffsetY = new FixedPrecisionCoordinates(Math.min(Math.max(MultitailArrow.MIN_TOP_BOTTOM_OFFSET.value, this.headOffsetY.add(fixedPrecisionOffset).value), this.height.sub(MultitailArrow.MIN_TOP_BOTTOM_OFFSET).value));
    ...
    value: function resizeTails(offset) {
      var fixedPrecisionOffset = FixedPrecisionCoordinates.fromFloatingPrecision(offset);
      var updatedLength = new FixedPrecisionCoordinates(Math.max(this.tailLength.sub(fixedPrecisionOffset).value, MultitailArrow.MIN_TAIL_LENGTH.value));
    ...
    value: function normalizeTailPosition(proposedPosition, tailId) {
      ...
      var positionCenter = FixedPrecisionCoordinates.fromFloatingPrecision(tailMinDistance.center);
      var positionDistance = FixedPrecisionCoordinates.fromFloatingPrecision(tailMinDistance.distance);
    ...
    value: function moveTail(offset, second, normalize) {
      var offsetFixedPrecision = FixedPrecisionCoordinates.fromFloatingPrecision(offset);
      var minHeight = new FixedPrecisionCoordinates(Math.max(MultitailArrow.MIN_TAIL_DISTANCE.multiply(this.tailsYOffset.size + 1).value, MultitailArrow.MIN_HEIGHT.value));
      var tailsOffset = Array.from(this.tailsYOffset.values());
      tailsOffset.sort(function (a, b) {
        return a.value - b.value;
      });
      var lastTail = tailsOffset.at(-1) || new FixedPrecisionCoordinates(0);
      var firstTail = tailsOffset.at(0) || new FixedPrecisionCoordinates(Infinity);
      var closestTopLimit = new FixedPrecisionCoordinates(Math.min(firstTail.sub(MultitailArrow.MIN_TAIL_DISTANCE).value, this.headOffsetY.sub(MultitailArrow.MIN_TOP_BOTTOM_OFFSET).value));
      var closestBottomLimit = new FixedPrecisionCoordinates(Math.max(lastTail.add(MultitailArrow.MIN_TAIL_DISTANCE).value, this.headOffsetY.add(MultitailArrow.MIN_TOP_BOTTOM_OFFSET).value));
      if (typeof second === 'number') {
        var originalValue = this.tailsYOffset.get(second);
        var updatedHeight = new FixedPrecisionCoordinates(Math.max(MultitailArrow.MIN_TAIL_DISTANCE.value, Math.min(originalValue.add(offsetFixedPrecision).value, this.height.sub(MultitailArrow.MIN_TAIL_DISTANCE).value)));
        if (normalize) {
        ...
        var _updatedHeight = new FixedPrecisionCoordinates(Math.max(minHeight.value, this.height.add(offsetFixedPrecision).value, closestBottomLimit.value));
        var _realOffset = _updatedHeight.sub(this.height);
        this.height = _updatedHeight;
        return _realOffset.getFloatingPrecision();
      } else {
        var _realOffset2 = new FixedPrecisionCoordinates(Math.min(offsetFixedPrecision.value, closestTopLimit.value, this.height.sub(minHeight).value));
    ...
    value: function move(offset) {
      this.spineTopX = this.spineTopX.add(FixedPrecisionCoordinates.fromFloatingPrecision(offset.x));
      this.spineTopY = this.spineTopY.add(FixedPrecisionCoordinates.fromFloatingPrecision(offset.y));
    }
  }, {
    key: "toKetNode",
    ...
    value: function fromTwoPoints(topLeft, bottomRight) {
      var center = Vec2.centre(topLeft, bottomRight);
      var tailLength = FixedPrecisionCoordinates.fromFloatingPrecision(Math.max((bottomRight.x - topLeft.x) / 3, MultitailArrow.MIN_TAIL_LENGTH.getFloatingPrecision()));
      var topSpineX = FixedPrecisionCoordinates.fromFloatingPrecision(topLeft.x).add(tailLength);
      var topSpineY = FixedPrecisionCoordinates.fromFloatingPrecision(topLeft.y);
      var height = FixedPrecisionCoordinates.fromFloatingPrecision(Math.max(bottomRight.y - topLeft.y, MultitailArrow.MIN_HEIGHT.getFloatingPrecision()));
      var headOffsetX = FixedPrecisionCoordinates.fromFloatingPrecision(bottomRight.x).sub(topSpineX);
      var headOffsetY = FixedPrecisionCoordinates.fromFloatingPrecision(center.y).sub(topSpineY);
      return new MultitailArrow(topSpineX, topSpineY, height, headOffsetX, headOffsetY, tailLength, new Pool());
    }
  }, {
    key: "validateKetNode",
    value: function validateKetNode(ketFileData) {
      ...
      var _spine$pos = _slicedToArray(spine.pos, 2),
        spineStart = _spine$pos[0],
        spineEnd = _spine$pos[1];
      var spineStartX = FixedPrecisionCoordinates.fromFloatingPrecision(spineStart.x);
      var spineStartY = FixedPrecisionCoordinates.fromFloatingPrecision(spineStart.y);
      var spineEndX = FixedPrecisionCoordinates.fromFloatingPrecision(spineEnd.x);
      var spineEndY = FixedPrecisionCoordinates.fromFloatingPrecision(spineEnd.y);
      var headX = FixedPrecisionCoordinates.fromFloatingPrecision(head.position.x);
      var headY = FixedPrecisionCoordinates.fromFloatingPrecision(head.position.y);
    ...
    value: function getConstructorParamsFromKetNode(ketFileNode) {
      var data = getNodeWithInvertedYCoord(ketFileNode.data);
      var _data$spine$pos = _slicedToArray(data.spine.pos, 2),
        spineStart = _data$spine$pos[0],
        spineEnd = _data$spine$pos[1];
      var head = data.head.position;
      var spineTopX = FixedPrecisionCoordinates.fromFloatingPrecision(spineStart.x);
      var spineTopY = FixedPrecisionCoordinates.fromFloatingPrecision(spineStart.y);
      var height = FixedPrecisionCoordinates.fromFloatingPrecision(spineEnd.y).sub(spineTopY);
      var headOffsetX = FixedPrecisionCoordinates.fromFloatingPrecision(head.x).sub(spineTopX);
      var headOffsetY = FixedPrecisionCoordinates.fromFloatingPrecision(head.y).sub(spineTopY);
      var tailsYOffset = new Pool();
      var tails = _toConsumableArray(data.tails.pos);
      tails.sort(function (a, b) {
        return a.y - b.y;
      });
      var tailsLength = spineTopX.sub(FixedPrecisionCoordinates.fromFloatingPrecision(tails[0].x));
      tails.slice(1, -1).forEach(function (tail) {
        tailsYOffset.add(FixedPrecisionCoordinates.fromFloatingPrecision(tail.y).sub(spineTopY));
      });
      return {
        spineTopX: spineTopX,
        spineTopY: spineTopY,
    ...
    value: function fromFloatingPointCoordinates(spineTop, height, headOffset, tailLength, tailsYOffset) {
      ...
      return new MultitailArrow(FixedPrecisionCoordinates.fromFloatingPrecision(spineTop.x), FixedPrecisionCoordinates.fromFloatingPrecision(spineTop.y), FixedPrecisionCoordinates.fromFloatingPrecision(height), FixedPrecisionCoordinates.fromFloatingPrecision(headOffset.x), FixedPrecisionCoordinates.fromFloatingPrecision(headOffset.y), FixedPrecisionCoordinates.fromFloatingPrecision(tailLength), tailsYOffsetFixedPrecision);
    }
  }, {
    key: "getReferencePositions",
    value: function getReferencePositions(spineTopX, spineTopY, height, headOffsetX, headOffsetY, tailLength, tailsYOffset) {
      ...
      tailsYOffset.forEach(function (tailYOffset, key) {
        tails.set(key, new Vec2(tailX.getFloatingPrecision(), spineTopY.add(tailYOffset).getFloatingPrecision()));
      });
      return {
        head: new Vec2(spineTopX.add(headOffsetX).getFloatingPrecision(), spineTopY.add(headOffsetY).getFloatingPrecision()),
        topTail: new Vec2(tailX.getFloatingPrecision(), spineTopY.getFloatingPrecision()),
        bottomTail: new Vec2(tailX.getFloatingPrecision(), bottomY.getFloatingPrecision()),
        topSpine: new Vec2(spineTopX.getFloatingPrecision(), spineTopY.getFloatingPrecision()),
        bottomSpine: new Vec2(spineTopX.getFloatingPrecision(), bottomY.getFloatingPrecision()),
        tails: tails
      };
    }
  }, {
    ...
    value: function getParametersForKetNode(spineTopX, spineTopY, headOffsetX, headOffsetY, tailLength, tailsYOffset, height, center, isInitiallySelected) {
      var head = new Vec2(spineTopX.add(headOffsetX).getFloatingPrecision(), spineTopY.add(headOffsetY).getFloatingPrecision());
      var bottomY = spineTopY.add(height);
      var spine = [new Vec2(spineTopX.getFloatingPrecision(), spineTopY.getFloatingPrecision()), new Vec2(spineTopX.getFloatingPrecision(), bottomY.getFloatingPrecision())];
      var tailX = spineTopX.sub(tailLength);
      ...
      var convertTail = function convertTail(y) {
        return new Vec2(tailX.getFloatingPrecision(), y.getFloatingPrecision());
      };
...
_defineProperty(MultitailArrow, "KET_MIN_DISTANCE", FixedPrecisionCoordinates.fromFloatingPrecision(0.01));
_defineProperty(MultitailArrow, "MIN_TAIL_DISTANCE", FixedPrecisionCoordinates.fromFloatingPrecision(0.35));
_defineProperty(MultitailArrow, "MIN_HEAD_LENGTH", FixedPrecisionCoordinates.fromFloatingPrecision(0.5));
_defineProperty(MultitailArrow, "MIN_TAIL_LENGTH", FixedPrecisionCoordinates.fromFloatingPrecision(0.4));
_defineProperty(MultitailArrow, "MIN_TOP_BOTTOM_OFFSET", FixedPrecisionCoordinates.fromFloatingPrecision(0.15));
_defineProperty(MultitailArrow, "MIN_HEIGHT", FixedPrecisionCoordinates.fromFloatingPrecision(0.5));
_defineProperty(MultitailArrow, "TOP_TAIL_NAME", 'topTail');
_defineProperty(MultitailArrow, "BOTTOM_TAIL_NAME", 'bottomTail');
_defineProperty(MultitailArrow, "TAILS_NAME", 'tails');

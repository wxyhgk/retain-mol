...
export const Settings = ({ isModalOpen, onClose }: RequiredModalProps) => {
  ...
  const handleSettingsChange = (partial: Partial<SettingsFormValue>) => {
  ...
  const handleApply = async () => {
    ...
    setIsLoading(true);
    try {
      const coreSettings = normalizeSettingsForCore(currentSettings);
      await settingsService.updateSettings(coreSettings);
      setInitialSettings(currentSettings);
      onClose();
    } catch (error) {
      KetcherLogger.error('Failed to save settings:', error);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleReset = async () => {
    ...
    try {
      ...
      const formDefaults = normalizeSettingsForForm(coreDefaults);
      setCurrentSettings(formDefaults);
      setInitialSettings(formDefaults);
    } catch (error) {
      KetcherLogger.error('Failed to reset settings:', error);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleACSStyle = async () => {
    ...
    try {
      ...
      KetcherLogger.error('Failed to apply ACS style:', error);
    } finally {
      setIsLoading(false);
    }
  };

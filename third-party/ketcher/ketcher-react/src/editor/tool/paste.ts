...
import {
  dropAndMerge,
  type DropAndMergeContext,
  type MergeItems,
} from './helper/dropAndMerge';
...
type PasteToolContext = DropAndMergeContext &
  Pick<IToolContext, 'event' | 'findItem' | 'findMerge' | 'tool'> & {
    lastEvent: Event | null;
  };

interface PasteDragContext {
  xy0: Vec2;
...
const debouncedSetAndHoverMergeItems = debounce(function (
  editor: PasteToolContext,
  pasteItems: SelectedItems,
  pasteToolInstance: PasteTool,
  needResetTool = false,
) {
  const mergeItems = getItemsToFuse(
    editor.render.ctab.molecule,
    editor.findMerge(pasteItems, ['atoms', 'bonds']),
  );
  editor.hover(
...
class PasteTool implements Tool {
  private readonly editor: PasteToolContext;
  private readonly struct: Struct;
  private action: Action | null | undefined;
  private dragCtx: PasteDragContext | undefined;
  ...
  public setMergeItems(mergeItems: MergeItems | null) {

...
export type DropAndMergeContext = Pick<
  IToolContext,
  'explicitSelected' | 'hover' | 'render' | 'selection' | 'struct' | 'update'
> &
  Partial<Pick<IToolContext, 'options'>>;

export type MergeItems = Pick<ItemsToFuse, 'atoms' | 'bonds'> &
  Partial<Pick<ItemsToFuse, 'atomToFunctionalGroup'>>;

export function dropAndMerge(
  editor: DropAndMergeContext,
  mergeItems: MergeItems | null | undefined,
  action?: Action | null,
  copyAction?: Action,
): Action {
  ...
  for (const bondId of bonds) {
    ...
    if (rebond) {
      ReBond.bondRecalc(
        rebond,
        restruct,
        editor.options?.() ?? editor.render.options,
      );
    }
  }
 ...
 */
function extractAttachmentAtom(
  mergeItems: MergeItems,
  editor: Pick<DropAndMergeContext, 'render' | 'struct'>,
) {

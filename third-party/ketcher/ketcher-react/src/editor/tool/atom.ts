...
type AtomToolProperties = Partial<AtomAttributes> &
  Pick<AtomAttributes, 'label'> & { type?: string };

type AtomToolContext = Pick<
  IToolContext,
  'event' | 'findItem' | 'hover' | 'render' | 'selection' | 'struct' | 'update'
> & {
  hoverIcon: {
    fill: string;
    label: string;
...
class AtomTool implements Tool {
  private readonly editor: AtomToolContext;
  private readonly atomProps: AtomToolProperties;
  private dragCtx: AtomDragContext | undefined;

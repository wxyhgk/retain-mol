...
type SelectionToolEvents = IToolContext['event'] & {
  ...
  elementEdit: {
    ...
    dispatch(payload: TextEditPayload): Promise<{ content?: string }>;
  };
};

type SelectionToolBaseContext = Pick<
  IToolContext,
  | 'errorHandler'
  | 'explicitSelected'
  | 'findItem'
...
export type SelectionStateContext = Pick<
  SelectionToolContext,
  'render' | 'selection' | 'struct'
>;

export type RotationToolContext = Pick<
  SelectionToolContext,
  | 'event'
  | 'explicitSelected'
  | 'findMerge'
...
export type SelectionRotationContext = RotationToolContext &
  Pick<IToolContext, 'tool'>;

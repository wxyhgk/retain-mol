...
import { createCopyOfSelected } from 'src/ui/action/createCopyOfSelected';
import type {
  EditableSelectionToolContext,
  SelectionToolContext,
} from './selectionToolContext';
...
class SelectTool implements Tool {
  ...
  private readonly editor: EditableSelectionToolContext;
  ...
  mousemove(event: PointerEvent) {
    ...
    if (isSelectionMoveDragContext(dragCtx)) {
      ...
      // Fix viewBox drift: pause model move while viewport is panning.
      // fromMultipleMove and moveViewBox both depend on viewBox.minX/minY;
      // writing both in the same frame shifts pageToModel baseline by
      // MOVE_STEP(5px)/zoom and causes jitter. When close to edge, pan only.
      const [isCloseToEdge, closeEdges] = this.isCloseToEdgeOfCanvas(event);
      if (isCloseToEdge) {
        const before = { ...this.editor.render.viewBox };
        this.moveViewBox(closeEdges);
        const after = this.editor.render.viewBox;
        const dx = after.minX - before.minX;
        const dy = after.minY - before.minY;
        if (dx !== 0 || dy !== 0) {
          const scale = this.editor.render.options.microModeScale;
          selectionDragCtx.xy0 = selectionDragCtx.xy0.add(
            new Vec2(dx / scale, dy / scale),
          );
        }
        return true;
      }
      ...
      const delta = CoordinateTransformation.pageToModel(
        event,
        editor.render,
      ).sub(selectionDragCtx.xy0);
      selectionDragCtx.action = fromMultipleMove(restruct, expSel, delta);
      ...
      selectionDragCtx.mergeItems = getItemsToFuse(
        ...
        editor.findMerge(visibleSelectedItems, ['atoms', 'bonds']),
      );
      editor.hover(getHoverToFuse(selectionDragCtx.mergeItems));

      editor.update(selectionDragCtx.action, true);
      // per-frame incremental translate, no throttle
      editor.rotateController.reposition(delta);
    } else {
      ...
      dialog
        .then(({ content }) => {
          if (content == null || content === '') {
            ...
            throw new Error('Text editor returned invalid content');
  ...
  private handleMoveCloseToEdgeOfCanvas() {
    ...
    if (isCloseToSomeEdgeOfScreen) {
      // RAF edge-pan: viewport only, do not call mousemove here to avoid
      // same-frame fromMultipleMove + setViewBox double write (5px drift).
      // Model moves are handled by real mousemove; when close that path
      // also pauses model and pans only (see mousemove guard above).
      // When dragging a selection, mousemove already pans per frame and
      // compensates dragCtx.xy0 with viewBox delta. RAF pan would double
      // speed (mousemove +5 plus RAF +5 per frame), so skip it for
      // selection moves — keeps single pan source and avoids xy0 stale jump.
      const isSelectionDrag =
        this.dragCtx && isSelectionMoveDragContext(this.dragCtx);
      if (!isSelectionDrag) {
        this.moveViewBox(closeEdges);
      }
    }
...
function getHoverTarget(
  ...
  editor: Pick<SelectionToolContext, 'render'>,
): HoverTarget | null {
  if (item?.map !== 'frags') {
    return item;
  }

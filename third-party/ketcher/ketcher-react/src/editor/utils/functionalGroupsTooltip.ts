...
export interface FunctionalGroupsTooltipContext {
  ...
  findItem(event: Event, maps: string[] | null): { id: number } | null;
  struct(): Struct;
}

const showTooltipTimers = new WeakMap<
  FunctionalGroupsTooltipContext,
...
function makeStruct(editor: FunctionalGroupsTooltipContext, sGroup: SGroup) {
...
function hideTooltip(editor: FunctionalGroupsTooltipContext) {
  editor.event.showInfo.dispatch(null);

  const timer = showTooltipTimers.get(editor);
  if (timer !== undefined) {
    clearTimeout(timer);
    showTooltipTimers.delete(editor);
  }
}
...
function showTooltip(
  ...
  showTooltipTimers.set(editor, timer);
}

...
export function moveSelectedItems(
  ...
  const selectedItems = ctx.explicitSelected();
  ...
  ctx.update(action, false);
  const isClose = isCloseToTheEdgeOfCanvas(selectedItems, ctx, key);
  if (isClose) {
    const moveStep = isShiftPressed
      ? getFasterStep(destinationVectorInCanvas)
      : destinationVectorInCanvas;
    ctx.render.setViewBox((prev) => ({
      ...prev,
...
function isCloseToTheEdgeOfCanvas(
  ...
  if (
    ...
    const getScreenCoordinates = (atom: ReAtom) =>
      Scale.modelToCanvas(atom.a.pp, ctx.render.options);
    ...
    const viewBox = ctx.render.viewBox;
    const isCloseToTop =

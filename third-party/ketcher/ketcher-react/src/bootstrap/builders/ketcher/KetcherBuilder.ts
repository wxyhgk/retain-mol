...
import { IndigoProvider } from 'src/bootstrap/providers';
...
class KetcherBuilder {
  ...
  async appendUiAsync(
    ...
    const detachMoleculeReader = attachMoleculeReader(
      ketcherProvider.getKetcher(ketcherId),
      editor,
    );
    const cleanupUi = cleanup as ReturnType<typeof initApp> | null;
    return {
      cleanup: () => {
        detachMoleculeReader();
        cleanupUi?.();
      },
      setServer,
    };

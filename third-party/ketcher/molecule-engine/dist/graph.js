...
export function validateGraph(snapshot) {
    ...
    for (const bond of snapshot.bonds) {
        ...
        if (endpoints.has(key)) {
            return failure('duplicate-bond', 'Only one bond may connect an atom pair.', {
                references: [bond.begin, bond.end],
            });
        }
        endpoints.add(key);
...
export function buildDraft(snapshot, request, idPrefix, reservedIds) {
    ...
    for (const [commandIndex, command] of request.commands.entries()) {
        if ('ref' in command && refs.has(command.ref)) {
            return failure('duplicate-reference', 'Request-local references must be unique across atoms and bonds.', { commandIndex, references: [command.ref] });
        }
        switch (command.op) {
            case 'atom.add': {
            ...
            case 'atom.remove': {
                ...
                if (incident.length > 0 && command.incidentBonds === 'reject') {
                    return failure('atom-has-bonds', 'Remove incident bonds explicitly or select incidentBonds: remove.', {
                        commandIndex,
            ...
            case 'bond.add': {
                ...
                if (endpoints.has(key)) {
                    return failure('duplicate-bond', 'Only one bond may connect an atom pair; update its order instead.', { commandIndex, references: [begin.value, end.value] });
                }

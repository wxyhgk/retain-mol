import { DOCUMENT_SCHEMA, EDIT_SCHEMA, PROFILE, validateCancelRequest, validateCommitRequest, validateDocumentSnapshot, validateEditRequest, validateHistoryRequest, } from 'molecule-contracts';
import { buildDraft, diffGraph, MAX_ATOMS, MAX_BONDS, validateGraph, } from './graph.js';
...
function createNamespace() {
    if (sessionSequence === Number.MAX_SAFE_INTEGER) {
        return failure('limit-exceeded', 'The runtime session ID limit was reached.');
    }
    ...
    try {
        if (typeof runtime.crypto?.getRandomValues !== 'function') {
            return failure('unsupported-runtime', 'This runtime must provide Web Crypto getRandomValues for session IDs.');
        }
        ...
        return failure('unsupported-runtime', 'The runtime could not generate a Web Crypto session ID.');
    }
}
const warnings = freeze([
    {
...
function configure(options) {
    try {
        if (options === null ||
            ...
            return failure('invalid-request', 'Session options must be a plain object.');
        }
        ...
        for (const key of Reflect.ownKeys(options)) {
            ...
            if (typeof key !== 'string' ||
                ...
                return failure('invalid-request', 'Session options contain an unknown field or a non-data property.');
            }
        }
        ...
        if (restored.value.documentId !== options.documentId) {
            return failure('document-mismatch', 'The initial snapshot belongs to a different document.');
        }
        ...
        for (const [name, limit] of Object.entries(limits)) {
            if (!Number.isSafeInteger(limit) ||
                ...
                return failure('invalid-request', `${name} must be an integer between 1 and ${MAX_CACHE_ENTRIES}.`, { path: `/${name}` });
            }
        }
 ...
 */
export function createMoleculeSession(options) {
    ...
    const checkVersion = (documentId, baseRevision) => {
        if (documentId !== document.documentId) {
            return failure('document-mismatch', 'This session owns another document.');
        }
        if (baseRevision !== document.revision) {
            return failure('revision-conflict', 'Read the current document and prepare an edit against its revision.', { currentRevision: document.revision });
        }
        return success(true);
    };
    ...
    function prepareEdit(input) {
        ...
        if (pendingById.size >= maxPreparedEdits) {
            return failure('limit-exceeded', 'The pending draft limit was reached; commit or cancel a draft first.');
        }
    ...
    function commitEdit(input) {
        ...
        if (!pending) {
            return failure('prepared-not-found', 'The prepared edit is absent, cancelled, evicted or from another session.');
        }
    ...
    function moveHistory(input, direction) {
        ...
        if (entry.commitId !== request.expectedCommitId) {
            return failure('history-conflict', `The most recent edit available to ${direction} is different.`, { references: [entry.commitId], currentRevision: document.revision });
        }
        const revision = canAdvanceRevision();
        if (!revision.ok)
            return revision;

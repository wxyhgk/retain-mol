"""Repository tests for immutable, typed job input snapshots."""
...
from software.backend.jobs.models import Artifact, Job, JobInput, JobInputSnapshot
...
def _literal(snapshot_id: str, job_id: str, name: str, value: object) -> JobInputSnapshot:
...
def test_insert_and_list_all_snapshot_sources(tmp_path) -> None:
    repository = JobRepository(tmp_path / "jobs.sqlite")
    ...
    repository.insert_job_input_snapshots(input_snapshots)

    persisted = {
        snapshot.input_name: snapshot
        for snapshot in repository.list_job_input_snapshots("target-job")
    }
    assert persisted == {
        snapshot.input_name: snapshot for snapshot in input_snapshots
    }
    assert input_snapshots[0].model_dump(by_alias=True)["inputName"] == "charge"
    assert input_snapshots[1].model_dump(by_alias=True)["moleculeRevisionId"] == "revision-1"

    job_payload = _job("snapshot-job").model_copy(
        update={"input_snapshots": input_snapshots}
...
def test_unique_input_name_and_foreign_keys_are_enforced(tmp_path) -> None:
    repository = JobRepository(tmp_path / "jobs.sqlite")
    repository.create_job(_job("job-1"))
    repository.insert_job_input_snapshots(
...
def test_replace_is_atomic_and_scoped_to_one_job(tmp_path) -> None:
    ...
    repository.insert_job_input_snapshots([original, other])

    replacement = _literal("binding-replacement", "job-1", "multiplicity", 1)
    repository.replace_job_input_snapshots("job-1", [replacement])

    assert repository.list_job_input_snapshots("job-1") == [replacement]
    assert repository.list_job_input_snapshots("job-2") == [other]

    invalid = JobInputSnapshot(
        snapshot_id="binding-invalid",
        jobId="job-1",
        inputName="structure",
        sourceKind="artifact",
    ...
    with pytest.raises(sqlite3.IntegrityError):
        repository.replace_job_input_snapshots("job-1", [invalid])
    assert repository.list_job_input_snapshots("job-1") == [replacement]

    with pytest.raises(ValueError, match="requested job"):
        repository.replace_job_input_snapshots("job-1", [other])
